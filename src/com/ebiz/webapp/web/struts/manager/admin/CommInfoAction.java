...
public class CommInfoAction extends BaseAdminAction {
	@Override
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		String st_down_date = (String) dynaBean.get("st_down_date");
		String en_down_date = (String) dynaBean.get("en_down_date");
		...
		if (StringUtils.isNotBlank(st_down_date)) {
			entity.getMap().put("st_down_date", st_down_date);
		}
		if (StringUtils.isNotBlank(en_down_date)) {
			entity.getMap().put("en_down_date", en_down_date);
		}
		if (GenericValidator.isInt(is_sell)) {
			entity.setIs_sell(Integer.valueOf(is_sell));
		}
	...
	public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		// 不是审核的人时候需要更新商品的par_cls_id 和 root_cls_id
		if (StringUtils.isBlank(is_audit) && StringUtils.isNotBlank(cls_id)) {
			BaseClass baseClass = super.getBaseClass(Integer.valueOf(cls_id));
			if (null != baseClass) {
		...
		if (StringUtils.isNotBlank(group_price)) {
		...
		// 更新企业表中的qq记录
		if (StringUtils.isNotBlank(qq) && entity.getOwn_entp_id() != null) {
		...
		if (StringUtils.isNotBlank(id)) {
			...
			if (StringUtils.isNotBlank(audit_state)) {
				...
				if (Integer.valueOf(audit_state) == 1) {// 审核通过
					...
					commInfoQuery.setId(entity.getId());
					...
					super.createQrcode(Path, Jump_path, LogoFile, name, uploadDir, comm_id);// 生成二维码
					entity.setComm_qrcode_path(uploadDir + File.separator + comm_id + ".png");
				}
	...
	public ActionForward edittcfw(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (comm_id != null && StringUtils.isNotBlank(comm_id)) {
			CommInfo commInfo = new CommInfo();
	...
	public ActionForward savetcfw(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		String[] comm_weights = request.getParameterValues("comm_weight");
		...
		String[] group_price = request.getParameterValues("group_price");
		...
		if (null != tczh_prices) {
			for (int i = 0; i < tczh_prices.length; i++) {
				...
				if (ArrayUtils.isNotEmpty(group_price)) {
					ctp.setGroup_price(new BigDecimal(group_price[i]));
				}
				if (ArrayUtils.isNotEmpty(orig_prices)) {
					ctp.setOrig_price(new BigDecimal(orig_prices[i]));
				}
				if (ArrayUtils.isNotEmpty(comm_weights)) {
					ctp.setComm_weight(new BigDecimal(comm_weights[i]));
				}
	...
	public ActionForward downloadQrcode(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		String today_date = (String) dynaBean.get("today_date");
		String comm_type = (String) dynaBean.get("comm_type");
		String down_date_le = (String) dynaBean.get("down_date_le");
		...
		if (StringUtils.isNotBlank(down_date_le)) {
			entity.getMap().put("down_date_le", down_date_le + " 23:59:59");
		}
	...
	public ActionForward validateCommNotes(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		DynaBean dynaBean = (DynaBean) form;

		StringBuffer oper = new StringBuffer("{\"result\":");
		String comm_notes = (String) dynaBean.get("comm_notes");
		CommInfo entity = new CommInfo();
		entity.setComm_notes(comm_notes);
		entity.setIs_del(0);
		int recordCount = super.getFacade().getCommInfoService().getCommInfoCount(entity);
		if (recordCount <= 0) { // 型号不存在
			oper.append(false);
		} else if (recordCount > 0) { // 型号重复
			oper.append(true);
		}
		oper.append("}");
