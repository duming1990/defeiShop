...
<div align="center">
  <html-el:form action="BaseCsAjax.do">
    <html-el:hidden property="method" value="getFreightInfoList" />
    <html-el:hidden property="canAdd" styleId="canAdd"/>
    ...
    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="tableClassSearch">
      <tr>
        <th align="left" nowrap="nowrap">
          	物流模板名称：
          <html-el:text property="fre_title_like" styleClass="webinput" maxlength="50"/>
          &nbsp;
  ...
  <div>
  <c:if test="${not empty af.map.canAdd}">
  <div style="padding-bottom:5px;float:left;">
	<button class="bgButtonFontAwesome" type="button" onclick="location.href='${ctx}/manager/customer/MyFreight.do?method=add&fromChoose=true';" ><i class="fa fa-plus-square"></i>添加</button>
  </div>
  </c:if>
  <table width="100%" border="0" cellpadding="1" cellspacing="1" class="backTable">
          <tr class="tite2">
            <th align="center" nowrap="nowrap">物流模板名称</th>
            <th width="20%" align="center" nowrap="nowrap">操作</th>
          </tr>
          <c:forEach var="cur" items="${entityList}" varStatus="vs">
            <tr>
              <td align="left">${fn:escapeXml(cur.fre_title)} </td>
              <td align="center">
              <span class="bgButtonFontAwesome" onclick="returnInfo('${cur.id}','${cur.fre_title}');">
      ...
      <tr>
        <td height="40" align="center"><script type="text/javascript" src="${ctx}/commons/scripts/pager.js">;</script> 
          <script type="text/javascript">
            ...
            pager.addHiddenInputs("method", "getFreightInfoList");
            pager.addHiddenInputs("fre_title_like", "${fn:escapeXml(af.map.fre_title_like)}");
            pager.addHiddenInputs("canAdd", "${fn:escapeXml(af.map.canAdd)}");
...
function returnInfo(val0,val1){
	...
	W.document.getElementById("freight_id").value = val0;
	W.document.getElementById("fre_title").value = val1;
	api.close();
}
//]]></script>
