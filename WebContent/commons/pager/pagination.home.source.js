...
jQuery.fn.pagination = function(opts) {
	...
	return this.each(function() {
		...
		function pageSelectedAndSubmitForm(page_id, pageForm) {
			currentPage = page_id;
			drawLinks();
			var hrp = '<input name="pager.requestPage" type="hidden" value="' + (page_id + 1) +'" >';
			panel.after(hrp);
			pageForm.submit();
			return ;
		}
		...
		function drawLinks() {
			...
			var appendItem = function(page_id, appendopts) {
				...
				if (page_id == currentPage) {
					...
					var lnk = jQuery("<li><a href='javascript:;'>" + (appendopts.text) + "</a></li>").bind(
							"click", getClickHandler(page_id)).attr('href',
									opts.link_to.replace(/__id__/, page_id));
				}
				if ((currentPage == 0) && (appendopts.text == opts.prev_text) && appendopts.classes) {
					lnk.unbind().addClass(appendopts.classes);
				}
				//alert("[page_id]:"+page_id +"[np]:"+np +"[appendopts.text]:"+appendopts.text)
				//$("#info").append("[page_id]:"+page_id +"[np]:"+np +"[appendopts.text]:"+appendopts.text).append("<br/>")
				if ((currentPage == np - 1) && (appendopts.text == opts.next_text) && appendopts.classes) {
					lnk.unbind().addClass(appendopts.classes);
				}
			...
			//alert((currentPage+1)+"---"+interval+"---"+np);
			// 产生small说明
			// panel.append("<small>Page " + (currentPage + 1) +" of " + np + "</small> ");
			// panel.append("<li><a href='javascript:;'>Page " + (currentPage + 1) +" of " + np + "</a></li>");
			...
			if (interval[0] > 0 && opts.num_edge_entries > 0) {
				...
				if (opts.num_edge_entries < interval[0] && opts.ellipse_text) {
					...
					jQuery("<li class='active'><a href='javascript:;'>" + opts.ellipse_text + "</a></li>").appendTo(panel);
				}
			}
			// 产生内部的些链接
