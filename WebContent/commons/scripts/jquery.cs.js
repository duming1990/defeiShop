	...
	cs : function(url, reqParamName, reqParamValue, isHideNoOptionSelect, selectLen) {
		...
		$.getJSON(url, data, function(json){
	        ...
	        for (var i = 0; i < tvs.length; i++) {
	        	this_[0].options[this_[0].options.length] = new Option(tvs[i][0], tvs[i][1]);
	            if (selectedValue != null && selectedValue == tvs[i][1]) {
	            	this_[0].selectedIndex = this_[0].options.length - 1;
	            	isSelected = true;
	            }
	        } 
	        if (this_[0].options.length > 0) {
	        	this_.show();
	        }
	        
			if (isSelected && null != attrSubElement) {
				$subElement.cs(url, reqParamName, this_[0].options[this_[0].selectedIndex].value, isHideNoOptionSelect);
			}
		});
