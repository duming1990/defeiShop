...
Validator = {
	...
	Urls : /^(https|http):\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/,
	Url2 : /^(https|http):\/\/[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/,
	...
	MoneyVerify : /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/,//金钱验证
	English : /^[A-Za-z]+$/,
	Chinese :  /^[\u0391-\uFFE5|\·]+$/,
	...
	Validate : function(theForm, mode){
		...
		if(this.ErrorMessage.length > 1){
			...
			case 3 :
				...
				break;
			default :
				mui.toast(this.ErrorMessage.join("\n"));
				break;
			}
			return false;
		}
