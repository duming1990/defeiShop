...
//头部清空购物车信息
function drop_topcart_all(){
    $.getJSON(SITEURL+'/index.php?act=cart&op=delCartAll&callback=?', function(result){
        if(result.state){
            var obj = $('.head-user-menu .my-cart');
            //删除成功
            if(result.quantity == 0){
...
/* 服务站进货、加入购物车 */
function add_stock_cart(goods_id, is_agent) {
    var url = 'index.php?act=cart&op=add_stock';
    $.getJSON(url, {'goods_id':goods_id, 'quantity':1, 'is_agent':is_agent}, function(data) {
        if (data != null) {
            if (data.state) {
                // 头部加载购物车信息
                load_cart_information();
