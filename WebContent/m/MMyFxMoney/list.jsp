     ...
     <c:forEach var="cur" items="${userBiRecordlList}" varStatus="vs">
      	<li>
	          ...
	          <div class="info">
	            ...
	            <div class="ordermain">
	              <p>操作前增值券：<em>
	              <fmt:formatNumber var="bi" pattern="0.########" value="${cur.bi_no_before/rmb_to_fanxianbi_rate}" />${bi}
	              </em></p>
	            </div>
	            <div class="ordermain">
	              <p>本次增值券：<em>
	              ...
	              <p>操作后增值券：<em><fmt:formatNumber var="bi" pattern="0.########" value="${cur.bi_no_after/rmb_to_fanxianbi_rate}" />${bi}</em></p>
	            </div>
	            <div class="ordermain">
	              <p>订单号：<em>${cur.map.trade_index}</em></p>
	            </div>
	            <div class="ordermain">
	              <p>操作时间：<em>
...
function getData() {
	...
	$.ajax({
		type: "POST",
		url: app_path + "/m/MMyFxMoney.do?method=getListJson",
		...
		success: function(datas) {
			...
			if (datas.ret == 1) {
				...
				$.each(dataList, function(i,data){  
					...
					html += '<div class="ordermain">';
					html += '<p>操作前增值券：<em>¥'+data.bi_no_before/${rmb_to_fanxianbi_rate}+'</em></p>';
					html += '</div>';
					...
					html += '<p>本次增值券：<em><span class="'+ classSpan +'">'+ pre +'&nbsp;'+ data.bi_no/${rmb_to_fanxianbi_rate} +'</span></em></p>';
					html += '<p>操作后增值券：<em>'+data.bi_no_after/${rmb_to_fanxianbi_rate}+'</em></p>';
					html += '</div>';
