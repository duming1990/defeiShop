...
<div id="notEmptyCart">
 <c:url var="url" value="MMyNoCartInfo.do" />
  <html-el:form action="${url}" styleClass="formOrder" method="post" >
    <html-el:hidden property="method" value="step2"/>
    <html-el:hidden property="shipping_address_id" value="${dftAddress.id}"/>
   ...
   <c:url var="url" value="/m/MEntpInfo.do?id=${af.map.comm_id}" />
	<section class="common-items carts_tr">
		 <c:if test="${not empty dftAddress && commInfo.comm_type==20}">
          ...
          <c:if test="${empty dftAddress}">
            <div class="common-item" style="padding:.3rem 0;width:80%;margin:0 auto;">
              <c:url var="url" value="/m/MMyNoCartInfo.do?method=addAddr&comm_id=${commInfo.id}&comm_tczh_id=${commTczhPrice.id}" />
...
function getAddressList(shipping_address_id) {
	  var comm_id = ${commInfo.id};
	  var comm_tczh_id = ${commTczhPrice.id};
	  Common.loading();
	  window.setTimeout(function() {
	    location.href = "${ctx}/m/MMyNoCartInfo.do?method=addressList&shipping_address_id=" + shipping_address_id+"&comm_id="+comm_id
	    				+"&comm_tczh_id="+comm_tczh_id;
	  }, 1000);
	}
