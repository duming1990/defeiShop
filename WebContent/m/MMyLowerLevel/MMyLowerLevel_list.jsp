...
<body>
	...
	<div class="content">
		<c:if test="${not empty userInfo.ymid}">
			<div style="padding: .16rem .24rem .12rem;" class="tip-success">
				<i class="fa fa-user"></i> 我的推荐人：${userInfo.ymid}
			</div>
			<div class="list-view">
				<ul class="list-ul">
					<li>
						<div class="list-item">
							<div class="info">
								<div class="ordermain">
									<p>
										登录名称：<span class="tip-success">${fn:escapeXml(uipar.user_name)}</span>
									</p>
									<fmt:formatNumber var="score" value="${uipar.cur_score}"
										pattern="0.########" />
									<p>
										积分：<span class="tip-success">${score}</span>
									</p>
								</div>
								<div class="ordermain">
									<p>
										用户等级：<span class="tip-success"> <c:forEach
												items="${baseDataList}" var="keys">
												<c:if test="${uipar.user_level eq  keys.id}">${keys.type_name}</c:if>
											</c:forEach> </em>
									</p>
									<p>
										身份：<span class="tip-success"> <c:set var="shenfen"
												value="会员" /> <c:if test="${uipar.is_entp eq 1}">
												<c:set var="shenfen" value="商家" />
											</c:if> <c:if test="${uipar.is_fuwu eq 1}">
								...
								<div class="ordermain">
									<p>
										添加时间：<span class="tip-success"> <fmt:formatDate
												value="${uipar.add_date}" pattern="yyyy-MM-dd" />
										</span>
									</p>
								</div>
							</div>
		...
		<c:if test="${not empty entityList}">
			<div style="padding: .16rem .24rem .12rem;" class="tip-danger">
				<i class="fa fa-users"></i> 我的团队：${entityListSize}
			</div>
			<div class="list-view">
							...
							<div class="list-item">
								<div class="info">
									...
									<div class="ordermain">
										...
										<div class="title-value1">
											<em> 
											<fmt:formatDate value="${cur.map.userInfo.add_date}"
													pattern="yyyy-MM-dd" />
											</em>
										</div>
										</p>
	...
	<script type="text/javascript">
		...
		function getData() {
			...
			$.ajax({
						...
						data : 'startPage=' + page + '&par_level=' + ${par_level} + '&'+ $(".searchForm").serialize(),
						dataType : "json",
