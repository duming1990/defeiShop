...
<div class="list-view">
  <ul class="list-ul" id="ul_data">
      <c:forEach items="${entityList}" var="cur">
       <c:url var="url" value="/m/MTiXianHuoKuanBi.do?method=view&id=${cur.id}&mod_id=${af.map.mod_id}"/>
      	<li onclick="goUrl('${url}')">
	          ...
	          <div class="info">
	            ...
	            <div class="ordermain">
	              <p>手续费：<em><fmt:formatNumber var="bi" value="${fn:escapeXml(cur.cash_rate)}" pattern="0" />${bi}%</em></p>
	              <p>实际金额：<em>${fn:escapeXml(cur.cash_pay)}</em></p>
	            </div>
	            <div class="ordermain">
	              <p>审核状态：<em>
		            <c:choose>
...
$(document).ready(function(){
 var topBtnUrl = "${ctx}/m/MTiXianHuoKuanBi.do?method=add&mod_id=${af.map.mod_id}";
...
function getData() {
	...
	$.ajax({
		type: "POST",
		url: app_path + "/m/MTiXianHuoKuanBi.do?method=getListJson",
		...
		success: function(datas) {
			...
			if (datas.ret == 1) {
				...
				$.each(dataList, function(i,data){  
					var url = app_path + "/m/MTiXianHuoKuanBi.do?method=view&id="+ data.id +"&mod_id=${af.map.mod_id}";
