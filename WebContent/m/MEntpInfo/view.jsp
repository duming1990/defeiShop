	...
	<div id="contact_pic_mask" v-show="is_kefu"></div>
		...
		<div id="content1">
			...
			<div class="aui-product-content">
				...
				<div class="aui-real-price clearfix" v-show="datas.commInfo.comm_type != 20">
				...
				<div class="aui-product-title" style="padding: 1px 15px 0px 15px;" v-show="datas.commInfo.comm_type == 20">
					<h2 v-if="datas.commInfo.group_type == 2">
						<span>老带新拼团，由拼团发起者邀请新用户注册后，新用户可以加入拼团</span>
					</h2>
					<h2 v-if="datas.commInfo.group_type == 1">
						<span>普通拼团，所有用户都可以参加</span>
					</h2>
				</div>
				
					<div class="aui-product-coupon" v-show="datas.commInfo.comm_type==20">
						<a @click="getGroupOrderInfo();" class="aui-address-cell aui-fl-arrow aui-fl-arrow-clear">
							<div class="aui-address-cell-bd" style="color: #ea1010">正在拼</div>
							<div class="aui-address-cell-ft">点击查看</div>
						</a>
					</div>
				...
				<div class="aui-product-coupon" v-if="datas.commInfo &&  datas.commInfo.comm_notes">
					<a class="aui-address-cell aui-fl-arrow-clear">
						<div class="aui-address-cell-bd">预售说明</div>
						<div class="aui-address-cell-ft">{{datas.commInfo.comm_notes}}</div>
					</a>
				</div>
				...
				<div class="aui-product-coupon" v-if="datas.entpInfo.is_coupons == 1">
					<a @click="showTip(0);" class="aui-address-cell aui-fl-arrow aui-fl-arrow-clear">
						<div class="aui-address-cell-bd" style="color: #ea1010">优惠券</div>
						<div class="aui-address-cell-ft">领取优惠券</div>
					</a>
				</div>
				<div class="aui-dri"></div>
				<div class="aui-product-evaluate" v-if="datas.entpInfo">
					<a :href="ctx + 'm/MEntpInfo.do?method=index&entp_id=' + datas.entpInfo.id" class="aui-address-cell aui-fl-arrow aui-fl-arrow-clear">
						<div class="aui-address-cell-bd">
							<div class="clearfix">
								<div class="aui-product-shop-img">
									<img :src="ctx + datas.entpInfo.entp_logo + '@s100x100'" />
								</div>
								<div class="aui-product-shop-text">
									<h4>{{datas.entpInfo.entp_name}}</h4>
									<span style="display: block;">精选商家 服务保障</span>
								</div>
							</div>
						</div>
					</a>
				...
				<div class="aui-slide-box">
					<div class="aui-slide-list swiper-container-comm">
						<ul class="aui-slide-item-list swiper-wrapper">
							<li class="aui-slide-item-item swiper-slide" v-if="datas.entityList" v-for="item in datas.entityList">
								<a :href="ctx + 'm/MEntpInfo.do?id=' + item.id" class="v-link">
									<img class="v-img" :src="ctx + item.main_pic + '@s200x200'" />
									<p class="aui-slide-item-title aui-slide-item-f-els">{{item.comm_name}}</p>
									<p class="aui-slide-item-info">
										<span class="aui-slide-item-price">¥{{item.sale_price | formatMoney}}</span>
									</p>
							</a></li>
						</ul>
					</div>
		...
		<footer class="aui-footer-product" id="s-actionBar-container">
			<div class="aui-footer-product-fixed">
				...
				<div class="aui-footer-product-action-list"  v-show="datas.commInfo.comm_type != 10 && datas.commInfo.comm_type != 20">
				...
				<div class="aui-footer-product-action-list" v-show="datas.commInfo.comm_type == 20">
					<!-- 				    <input type="hidden" name="pd_stock" id="pd_stock" v-model="datas.commInfo.inventory"/> -->
					<input type="hidden" name="comm_tczh_id" id="comm_tczh_id" v-model="comm_tczh_id" />
					 <a class="yellow-color" v-show="isLogin " @click="showBuyTip(1);" style="font-size:10px">￥{{datas.commInfo.sale_price | formatMoney}}单独购买</a>
					 <a class="red-color" v-show="isLogin" @click="isPt(1);" style="font-size:10px">￥{{datas.commInfo.map.commTczhPriceList[0].group_price | formatMoney}}我要开团</a>
					 <c:url var="url" value="/m/MIndexLogin.do?comm_id=${af.map.id}" />
					 <a class="yellow-color" v-show="!isLogin " onclick="toLogin('${url}');" style="font-size:10px">￥{{datas.commInfo.sale_price | formatMoney}}单独购买</a>
					 <a class="red-color" v-show="!isLogin" onclick="toLogin('${url}');" style="font-size:10px">￥{{datas.commInfo.map.commTczhPriceList[0].group_price | formatMoney}}我要开团</a>
				</div>
				
				<div class="aui-footer-product-action-list" v-show="datas.commInfo.comm_type == 10">
					<input type="hidden" name="comm_tczh_id" id="comm_tczh_id" v-model="comm_tczh_id" />
					 <a class="red-color" v-show="isLogin" @click="showBuyTip(1);" style="width: 100%;">立即购买</a>
					 <c:url var="url" value="/m/MIndexLogin.do?comm_id=${af.map.id}" />
					 <a class="red-color" v-show="!isLogin" onclick="toLogin('${url}');" style="width: 100%;">立即购买</a>
				</div>
		...
		<div class="mui-cover" id="s-decision-wrapper">
		   <div class="body">
			...
			<div class="number">
			  <h2>数量</h2>
				<div class="content">
				  <div class="mui-numbox" v-if="isPtFlag != 1">
					...
					<button class="mui-btn mui-numbox-btn-plus" type="button" @click="calcCartMoney(1);" v-if="isPtFlag != 1">+</button>
				  </div>
				  <div class="mui-numbox" v-if="isPtFlag == 1">
					<input class="mui-numbox-input" disabled="disabled" value="1"/>
				  </div>
						
						<div v-if="isPtFlag == 1">拼团商品只能购买一个</div>
					</div>
				</div>
				<div class="sku-dtips" v-show="jd_no_stock == true">提示:
						<span style="color:red;">您的默认收货地址，京东配送该区域暂无库存</span>
					</div>
			</section>
			</div>
			<div class="summary">
				...
				<div class="main">
					<div class="priceContainer">
						<span v-show="isPtFlag == 0" class="price" id="sale_price">{{datas.commInfo.sale_price | formatMoney}}</span>
						<span v-show="isPtFlag == 1" class="price" id="sale_price">{{datas.commInfo.map.commTczhPriceList[0].group_price | formatMoney}}</span>
					</div>
			...
			<div class="option mui-flex">
				...
				<!-- <span v-show="datas.commInfo.comm_type = 20"> -->
				<button class="ok cell" v-show="isLogin" @click="addCartOrBuy(false);">确定</button>
				
				<!-- <button class="ok cell" v-show="isLogin && datas.commInfo.comm_type=20" @click="addCartOrBuy(true);">确定</button> -->
			</div>
		</div>
		...
		<div class="mui-cover" id="a-pintuan-wrapper"
			style="top: 60%; background-color: #fdfafa;overflow-y: scroll;">
			<div>&nbsp</div>
			<div class="pindan" v-for="order in groupOrderList">
				<div><span style="margin-left:25px;">{{order.add_user_name}}的团还差{{order.map.leftCount}}人</span>
				<a class="join_button" v-show="isLogin" @click="joinGroup(order.id);">立即参团</a>
				<c:url var="url" value="/m/MIndexLogin.do?comm_id=${af.map.id}" />
				<a class="join_button" v-show="!isLogin" onclick="toLogin('${url}');">立即参团</a></div>
			</div>
			<a class="sback" @click="hideWrapper" style="position: absolute; top: 0px; right: 0px;">
			<i class="aui-icon aui-icon-close"></i></a>
		</div>
	...
	<script type="text/javascript">
		var vm = new Vue({
				...
				updated : function() {
					...
					methods : {
						joinGroup : function(leaderOrderId){
							location.href = vm.ctx+"m/MEntpInfo.do?method=getPtDetails&comm_id=${af.map.id}&leaderOrderId="+leaderOrderId;
						},
						getGroupOrderInfo : function(){
							var _this = this;
							Common.getData({
								route : 'm/MGroupBuy.do?comm_id=${af.map.id}',
								success : function(data) {
									if (data.code == 0) {
										mui.toast(data.msg);
										return false;
									} else if (data.code == 1) {
										vm.groupOrderList = data.datas.LeaderOrderInfoList;
										vm.code = data.code;
										_this.showPtTip();
									}
								},
							...
							Common.getData({
										...
										success : function(data) {
											if (data.code == 0) {
												...
												vm.jd_no_stock = data.datas.jd_no_stock;
											}
										},
										error : function() {
											mui.alert('好像出错了哦~');
						...
						hideWrapper : function() {
							...
							$("#a-pintuan-wrapper").removeClass("show");
							$("#s-decision-wrapper-cover").hide();
						...
						showPtTip : function() {
							$("#a-pintuan-wrapper").show().addClass("show");
							$("#a-pintuan-wrapper-cover").show();
							$("#s-pintuan-wrapper").removeClass("show");
							$("#s-pintuan-wrapper-cover").hide();
						},
						isPt : function(flag) {
							if(flag == 1){
							//开团购买前查询该用户订单下是否存在该商品未完成的拼团的订单
								Common.getData({
									route : 'm/MGroupBuy.do?method=checkOrderOfUser&comm_id=${af.map.id}',
									success : function(data) {
										if (data.code == 1) {//存在未完成订单，不可再次开团
											mui.toast(data.msg);
											return false;
										} else {
											$("#s-decision-wrapper").show().addClass("show");
					   ...
					   btnSearch:function(){
						...
						addCartOrBuy : function(noCart) {
							...
							var isYs = 0;
							if(vm.datas.commInfo.comm_type == 10){
								isYs = 1;
								vm.cart_type = 30;
							}
											...
											success : function(data) {
												if (vm.addCartOrBuyFlag == 1) {
													if (noCart) {
														...
														if(isYs == 1){
															location.href = app_path+ "/m/MMyCartInfo.do?method=step1&isYs=" + isYs;
														}else if (vm.isPtFlag == 1){
															...
															+ "&isPt=" + vm.isPtFlag+"&isLeader="+vm.isLeader+"&leaderOrderId="+vm.leaderOrderId+"&cart_id="+data.result;
														}else{
															location.href = app_path+ "/m/MMyCartInfo.do?cart_id="+data.result;
														}
