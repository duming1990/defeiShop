 ...
 <jsp:include page="../_header.jsp" flush="true" />
  ...
  <div class="content bg-white">
    ...
    <html-el:form action="/MIndexLogin.do" method="post" styleClass="form_login">
      <html-el:hidden property="method" value="loginme" />
    ...
    <dl class="login-other">
      <dt>使用合作网站账号登录</dt>
      <dd class="box profx-list"> 
      <c:url var="qq" value="/IndexLogin.do?method=qq" />
      <c:url var="weibo" value="/IndexLogin.do?method=weibo" />
      <c:url var="weixin" value="/weixin/WeixinLogin.do?method=weixin"/>
      <c:if test="${isWeixin}">
      	<a href="${weixin}"><span class="fx3"></span></a> 
      </c:if>
      <a href="${qq}"><span class="fx1"></span></a> 
      <a href="${weibo}"><span class="fx2"></span></a> </dd>
    </dl>
  </div>
</div>
...
$(document).ready(function(){
	 ...
	 $("#loginSubmit").click(function(){
		...
		if(null == user_name || '' == user_name){
			mui.toast("请输入用户名",1000);
			return false;
		}else if(null == password || '' == password){
			mui.toast("请输入密码",1000);
			return false;
		}
		Common.loading();
		f.submit();
