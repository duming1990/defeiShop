...
<form action="/m/MMyAccount" enctype="multipart/form-data" method="post" class="ajaxForm0">
    ...
    <input type="hidden" name="son_id" id="son_id" value="${son_id}" />
    <div class="set-site">
      <ul class="formUl">
      	<c:url var="url" value="/m/MEntpInfo.do?id=${ods.comm_id}" />
      	<li><a onclick="goUrl('${url}')">${ods.comm_name}<c:if test="${ods.comm_tczh_name}">[${ods.comm_tczh_name}]</c:if></a></li>
        <li><span>商品评分：</span>
	       <div>
        ...
        <li>
        <textarea id="content" name="content" placeholder="请输入回复内容..." class="textarea-new" >${content}</textarea>
        </li>
      </ul>
    </div>
					...
					$.ajax({
						...
						success: function(data) {
							if(data.ret == "1"){
								mui.toast(data.msg);
								window.setTimeout(function () {
									<c:if test="${af.map.comm_uid eq userInfo.id}">
									location.href="${ctx}/m/MMyComment.do?method=getCommentList&add_user_id="+${userInfo.id}+"&tip=1";
									</c:if>
									<c:if test="${af.map.comm_uid ne userInfo.id}">
									location.href="${ctx}/m/MMyComment.do?method=getCommentList&is_entp=true";
									</c:if>
