...
<div class="list-view">
  <ul class="list-ul" id="ul_data">
      <c:forEach items="${entityList}" var="cur">
       <c:url var="url" value="/m/MTiXianAidBi.do?method=view&id=${cur.id}&mod_id=${af.map.mod_id}"/>
      	<li onclick="goUrl('${url}')">
	          ...
	          <div class="info">
	            ...
	            <div class="ordermain">
	              <p>手续费：<em>¥${fn:escapeXml(cur.cash_rate)}</em></p>
	              <p>到账金额：<em>¥${fn:escapeXml(cur.cash_pay)}</em></p>
	            </div>
	            <div class="ordermain">
	              <p>审核状态：<em>
		            <c:choose>
...
$(document).ready(function(){
 var topBtnUrl = "${ctx}/m/MTiXianAidBi.do?method=add&mod_id=${af.map.mod_id}";
...
function getData() {
	...
	$.ajax({
		type: "POST",
		url: app_path + "/m/MTiXianAidBi.do?method=getListJson",
		...
		success: function(datas) {
			...
			if (datas.ret == 1) {
				...
				$.each(dataList, function(i,data){  
					var url = app_path + "/m/MTiXianAidBi.do?method=view&id="+ data.id +"&mod_id=${af.map.mod_id}";
					...
					html += '<p>到账金额：<em>¥'+data.cash_pay+'</em></p>';
					html += '</div>';
