...
<div class="content"> 
  <div class="myorder-ors">
  	<a href="${ctx}/m/MMyOrderEntp.do?method=list&order_type=${af.map.order_type}&mod_id=${af.map.mod_id}" id="ors_">全部</a>
	<a href="${ctx}/m/MMyOrderEntp.do?method=list&order_type=${af.map.order_type}&mod_id=${af.map.mod_id}&order_state=0" id="ors_0">待支付</a>
<%-- 	<c:if test="${(af.map.order_type eq 10) or (af.map.order_type eq 11)}"> --%>
	 <a href="${ctx}/m/MMyOrderEntp.do?method=list&order_type=${af.map.order_type}&mod_id=${af.map.mod_id}&order_state=10" id="ors_10">待发货</a>
	 <a href="${ctx}/m/MMyOrderEntp.do?method=list&order_type=${af.map.order_type}&mod_id=${af.map.mod_id}&order_state=20" id="ors_20">待收货</a>
	 <a href="${ctx}/m/MMyOrderEntp.do?method=list&order_type=${af.map.order_type}&mod_id=${af.map.mod_id}&order_state=40" id="ors_40">已收货</a>
<%-- 	</c:if> --%>
	<a href="${ctx}/m/MMyOrderEntp.do?method=list&order_type=${af.map.order_type}&mod_id=${af.map.mod_id}&order_state=50" id="ors_50">已完成</a>
  </div>
    ...
    <c:forEach items="${entityList}" var="cur">
     <div class="m item border-1px mar9">
        <div class="mt bdb-1px">
            ...
            <c:if test="${cur.order_state eq -10}"><a class="i-complete" onclick="orderDel('${cur.id}')"></a></c:if>
            <c:if test="${cur.map.orderAudit.audit_state eq -1 and cur.order_type eq 40}">
            <a onclick="Audit_detail('${cur.map.orderAudit.audit_desc}')" style="color: red;">审核不通过</a></c:if>
            <c:if test="${cur.map.orderAudit.audit_state eq 0 and cur.order_type eq 40}">待审核</c:if>
            </span>
        </div>
       <div class="mc">
        <c:url var="url" value="/m/MMyOrderDetail.do?method=view&order_id=${cur.id}&from=entp" />
		  <a onclick="goUrl('${url}');">
		    ...
		    <div class="imc-one">
			  <div class="imco-l">
			    ...
			    <div class="imco-r-content">
			        ...
			        <div class="imco-r">${curSon.map.commInfo.comm_name} 
  		 ...
  		 <div class="mb cf"><span class="true-payed">实付款:</span><span class="imb-num">¥<fmt:formatNumber value="${cur.order_money}" pattern="0.##"/></span>
  		  <c:if test="${cur.map.orderAudit.audit_state eq -1 and cur.order_type eq 40}">
  		  <c:url var="url" value="/m/MMyOrderEntp.do?method=edit&order_id=${cur.id}&mod_id=${af.map.mod_id}"></c:url>
  		  <div class="imb-btn-box"><a class="imb-btn" href="${url}">订单修改</a></div>
  		  </c:if>
  		 <c:choose>
         <c:when test="${cur.order_state eq 10}">
           <c:if test="${(cur.order_type eq 10) or (cur.order_type eq 11) or (cur.order_type eq 140)}">
<%--              <div class="imb-btn-box"><a class="imb-btn"  onclick="orderConfirm('${cur.id}');">订单确认</a></div> --%>
			<div class="imb-btn-box"><a class="imb-btn" onclick="orderFh('${cur.id}');">订单发货</a></div>
           </c:if>
         </c:when>
           <c:when test="${cur.order_state gt 10}">
            <c:if test="${cur.order_type eq 11}">
             <div class="imb-btn-box"><a class="imb-btn" onclick="kdcx(${cur.id})">物流信息查询</a></div>
             </c:if>
           </c:when>
  		</c:choose>
  	   </div>
  ...
  <form id="bottomPageForm" name="bottomPageForm" method="post" action="${ctx}/m/MMyOrderEntp.do">
...
// 					}
					...
					if(data.order_type == 11 || data.order_type == 10 || data.order_type == 140){
						html += '<div class="imb-btn-box"><a class="imb-btn" onclick="orderFh('+data.id+');">订单发货</a></div>';
					}
					if(data.order_state >= 20){
						if(data.order_type == 11){
						html += '<div class="imb-btn-box"><a class="imb-btn" onclick="kdcx('+data.id+');">查看物流</a></div>';
						}
					}
					html += '</div>';
					html += '</div> ';
