	...
	<section class="aui-myOrder-content" id="chooseAddr" style="margin-bottom: 65px;background: #eee;">
       <c:forEach items="${shippingAddressList}" var="cur">
		...
		<div class="common-item" style="padding:.3rem 0;width:80%;margin:0 auto;">
			<c:url var="url" value="/m/MWelfareCartInfo.do?method=addAddr&cart_ids=${cart_ids}"/>
...
$("#chooseAddr .aui-Address-box").each(function(index){
	$(this).click(function(){
		...
		window.setTimeout(function () {
		 if(cart_ids != null){
		  location.href="${ctx}/m/MWelfareCartInfo.do?method=step1&shipping_address_id=" + shipping_address_id + "&delivery_p_index=" + delivery_p_index +"&cart_ids="+cart_ids;
		 }else{
		  location.href="${ctx}/m/MWelfareCartInfo.do?method=step1&shipping_address_id=" + shipping_address_id + "&delivery_p_index=" + delivery_p_index;
		 }
		}, 500);
	});
});
