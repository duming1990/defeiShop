...
<jsp:include page="../_header.jsp" flush="true" />
 <c:url var="url" value="MWelfareCartInfo.do" />
...
<!-- 				</div> -->
				...
				<div class="common-item" id="payType0" style="display: none;">
					<label>
					<font class="o-icon o-chuxu" color="#333">福利卡</font>
						<input type="radio" name="pay_type" value="7" />
						<i></i>
					</label>
				</div>
				</section>
	...
	<c:if test="${cardInfo.card_cash gt 0}">
	<div style="margin-top: -7px;">
	<section class="common-items">
		<div class="common-item sel" id="bind_change">
			<a href="javascript:void(0);">
				<span class="item-label">福利卡余额：</span>
				<div class="item-content" style="text-align: left;">
					<b>¥<fmt:formatNumber value="${cardInfo.card_cash}" pattern="0.00"/></b>
				</div>
			</a>
		</div>
		<fmt:formatNumber var="card_dikou" value="${cardInfo.card_cash}" pattern="0.##"/>
		<c:if test="${(cardInfo.card_cash - order_money) ge 0}">
		<fmt:formatNumber var="card_dikou" value="${order_money}" pattern="0.##"/>
		</c:if>
		<div class="common-item sel" id="bind_change">
			<a href="javascript:void(0);">
				<span class="item-label">可抵扣：</span>
				<div class="item-content" style="text-align: left;">
				<b>¥<span id="kedi_money"><fmt:formatNumber value="${card_dikou}" pattern="0.00"/></span></b>
				</div>
			</a>
		</div>
	</section>
    ...
    <span style="line-height: 34px;min-width: 1.3rem;color: #8D8D8D;font-size: .25rem;">是否使用福利卡：</span>
     <div class="yu_e" style="margin-right: 5px;">
       <div class="mui-switch mui-switch-mini" id="vibrationSwitch" style="box-sizing: border-box;">
...
mui('.yu_e .mui-switch').each(function() {
	this.addEventListener('toggle', function(event) {
		var yu_e=parseFloat("${cardInfo.card_cash}");
		var order_money=$("#order_money").val();
		var trade_index="${trade_index}";
		if(event.detail.isActive){//使用福利卡抵扣
			
			$.ajax({
				type: "POST" , 
				url: "${ctx}/CsAjax.do", 
				data:"method=userCard&trade_index="+trade_index+"&order_money="+order_money,
				dataType: "json", 
		        async: true, 
			...
			$.ajax({
				type: "POST" , 
				url: "${ctx}/CsAjax.do", 
				data:"method=notUserCard&trade_index="+trade_index+"&order_money="+order_money,
				dataType: "json", 
		        async: true, 
		        error: function (request, settings) {alert(" 数据加载请求失败！ ");	$(".pay").attr("disabled", "true");}, 
		        success: function (data) {
...
function notUseCard(){
	...
	$.ajax({
		type: "POST" , 
		url: "${ctx}/CsAjax.do", 
		data:"method=notUserCard&trade_index="+trade_index,
		dataType: "json", 
        async: true, 
        error: function (request, settings) {alert(" 数据加载请求失败！ ");	$(".pay").attr("disabled", "true");}, 
        success: function (data) {
	...
	$.ajax({
        ...
        success: function (data) {
			if (data.ret == 1) {
				...
				$("#vibrationSwitch").addClass("mui-active");
			} else {
				mui.toast(data.msg,1000);
				$("#J-order-pay-button").addClass("disabled").removeAttr("onclick");
			} 
