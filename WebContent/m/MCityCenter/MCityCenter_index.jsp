...
<!-- 侧滑导航根容器11 -->
<div id="offCanvasWrapper" class="mui-off-canvas-wrap mui-draggable mui-slide-in" v-cloak>
	<!-- 菜单容器 -->
	<aside id="offCanvasSide" class="mui-off-canvas-right" style="background-color: white;top:44px;">
		<div id="offCanvasSideScroll" class="mui-scroll-wrapper" style="bottom: 50px;">
			<ul class="mui-scroll mui-table-view mui-table-view-chevron heights" style="width:30vw;float: right;border-left: 1px solid #eee;">
				<li class="mui-table-view-cells mui-collapse" @tap="openTown(item.value)" :class="{'mui-actives': index === 0}" :data-id="index+1" v-for="(item,index) in p_index_list">
					<a class="" href="#">{{item.text}}</a>
				</li>
			</ul>
		</div>
	</aside>
	...
	<div class="mui-inner-wrap mui-transitioning">
	   <header id="header" class="mui-bar mui-bar-nav" style="background-color:#f03e3d!important;">
		 ...
		 <a class="mui-icon mui-icon-bars mui-pull-right" href="#offCanvasSide" style="color:#fff;"></a>
		 <div class="mui-btn mui-btn-blue mui-btn-link mui-btn-nav mui-pull-left" id="city_name" @tap="openUrl(ctx+'m/MChangeCity.do')" style="color:#fff;margin-right:0px;">
		   定位中
		 </div>
			...
			<div class="mui-scroll">
				<div>
					<div class="cunhead">
						...
						<img class="bg" v-else :src="ctx + banner+'@s414x207'" />
						<h3 class="left-5" style="width: auto;padding-top:3px;">{{p_name}}
						<c:if test="${link_area eq 1}">
						<span class="commInfo_from" @tap="delHomePage()" id="delHomePage">取消主页</span>
						<span class="commInfo_from" style="display: none;" @tap="setHomePage()" id="setHomePage">设为主页</span>
						</c:if>
						...
						<span class="commInfo_from"  @tap="setHomePage()" id="setHomePage">设为主页</span>
						</c:if>
						
						
						</h3>
						<p class="cuncount left-5" style="font-size: 16px;">村民数量：{{datas.villageMemberCount}} &nbsp;&nbsp;昨日新增：+{{datas.yesterdayAddCount}}</p>
						<a class="mui-action-back"><i class="aui-icon aui-icon-back2 villageBack"></i></a>
					</div>
					<ul class="mui-table-view">
						<li class="mui-media cunzhu">
							<img class="mui-media-object mui-pull-left radius50" @tap="openUrl(ctx+ 'm/MUserCenter.do?method=index&user_id='+manageUser.id)" @click="openUser(manageUser.id);" v-if="manageUserLogo == ''" src="${ctx}/styles/images/user.png">
							<img class="mui-media-object mui-pull-left radius50" @tap="openUrl(ctx+ 'm/MUserCenter.do?method=index&user_id='+manageUser.id)" @click="openUser(manageUser.id);" v-else :src="ctx + manageUserLogo +'@s200x200'" onerror="this.src='${ctx}/styles/images/user.png'">
							<div class="mui-media-body">
								...
								<div class="cunlod mui-table-view-test">
									<span class="radius50" @tap="openUrl(ctx+ 'm/MServiceCenterInfo.do?p_index_like='+ p_index)">合伙人</span>
									<span v-if="'' != manageUser" class="radius50" @tap="openUrl(ctx+ 'm/MCityCenter.do?method=photo&id='+ manageUser.id)">市貌</span>
								</div>
							</div>
						</li>
					</ul>
					...
					<div class="toutiao" v-if="newsInfoList != ''">
						<i class="tticon"></i>
						<span @tap="openUrl(ctx+ 'm/MNewsInfo.do?add_uid='+ manageUser.id + '&mod_index=1600100103')"  v-if="index == 0" v-for="(item,index) in newsInfoList">{{item.title}}</span>
					</div>
					<div class="center-list-flex">
						<div class="center-list-flex center-list-par">
							<div class="center-list-content">
								<img src="${ctx}/m/styles/village/img/images/cun1.png" @tap="openUrl(ctx+ 'm/MCategory.do')" />
								<div>商品分类</div>
							</div>
							<div class="center-list-content">
<%-- 								<img src="${ctx}/m/styles/village/img/images/cun2.png" @tap="openUrl(ctx+ 'm/IndexMTsg.do?method=index&link_id=3')"/> --%>
								<img src="${ctx}/m/styles/village/img/images/cun2.png" @tap="openUrl(ctx+ 'm/MSearch.do?method=listPd&is_aid=1&p_index='+p_index)"/>
								<div>扶贫商品</div>
							</div>
							<div class="center-list-content">
					...
					<div class="center-list" v-if="datas.myJoinVillageList != ''">
						...
						<div class="aui-slide-box">
							<div class="aui-slide-list swiper-container-comm">
								<ul class="aui-slide-item-list swiper-wrapper">
									<li style="width:5.5rem!important;" class="aui-slide-item-item swiper-slide join-village-li" v-for="item in datas.myJoinVillageList">
										<a @tap="openUrl(ctx+ 'm/MVillage.do?method=index&id='+item.village_id)" class="v-link">
											<img :src="ctx + item.map.village_logo +'@s200x200'" class="v-img" onerror="this.src='${ctx}/styles/images/user.png'">
					...
					<div class="center-list" style="margin-top: 10px;" v-if="datas.poorInfoList != ''">
						...
						<div class="fen-ge-fu"></div>
						<div class="aui-slide-box">
							<div class="aui-slide-list swiper-container-poor">
								<ul class="aui-slide-item-list swiper-wrapper">
					...
					<div class="center-list" style="margin-top: 10px;" v-if="datas.saleRankList != ''">
						...
						<div class="aui-slide-box">
							<div class="aui-slide-list swiper-container-comm">
								<ul class="aui-slide-item-list swiper-wrapper">
									<li style="width:5.5rem!important;" class="aui-slide-item-item swiper-slide join-village-li" v-for="(item,index) in datas.saleRankList">
						...
						<div class="center-content">
							<div class="center-content2" v-for="(item,index) in datas.villageDynamicList">
								<div class="center-content21">
									<div class="center-content211" @tap="openUrl(ctx+ 'm/MUserCenter.do?method=index&user_id='+item.add_user_id)" @click="openUser(item.add_user_id);">
										<img v-if="null != item.map.user_logo && item.map.user_logo.indexOf('http://') == -1" :src="ctx + item.map.user_logo+'@s64x64'" onerror="this.src='${ctx}/styles/images/user64.png'" />
									...
									<div class="center-content212" @tap="openUrl(ctx+ 'm/MUserCenter.do?method=index&user_id='+item.add_user_id)" @click="openUser(item.add_user_id);">
...
// 			}, false);
			...
			var p_index = localStorage.getItem('p_index_service_info');
			 ...
			 mui.init({
				...
				function pullupRefresh() {
					Common.loading();
					Common.getData({
						route: '/m/MCityCenter.do?method=getAjaxDataPage',
						data: {
							id: "${af.map.id}",
							p_index: vm.p_index,
							pageSize: pageSize,
				...
				mui('.mui-off-canvas-right').on('tap', '.mui-collapse', function(e) {
					$(this).addClass("mui-actives").siblings().removeClass("mui-actives");
				});
	...
	updated: function() {
		...
		//$('.lightgallery').lightGallery({download:false,fullScreen:false,zoom:false});
...
// 			  },
	...
	methods: {
		getAjaxData: function(_p_index) {
			var p_index = localStorage.getItem('p_index_service_info');
			vm.p_index = p_index;
			vm.p_name = localStorage.getItem('p_name_service_info');
			Common.getData({
				route: '/m/MCityCenter.do?method=getAjaxData',
				data: {
					p_index: p_index,
				},
				success: function(data) {
					if(data.code == 0) {
						...
						if(null != vm.p_name && vm.p_name != "undefined") {
							$("#city_name").text(vm.p_name.substring(0, 3));
		...
		getPoorAjaxData:function(){
			...
			Common.getData({
				route: '/m/MCityCenter.do?method=getPoorAjaxData',
				data: {
					id: "${af.map.id}",
					p_index:vm.p_index,
					pageSize: 6,
...
// 						Common.hide();
						console.info("data.datas.poorInfoList.length:"+data.datas.poorInfoList.length);
		...
		openTown: function(p_index) {
			Common.getData({
				route: '/m/MCityCenter.do?method=openTown',
				data: {
					p_index: p_index,
				},
				success: function(data) {
		...
		setHomePage:function (){
			Common.getData({
				...
				data: {
					link_area: '/MCityCenter/MCityCenter_index.jsp',
				},
				success: function(data) {
