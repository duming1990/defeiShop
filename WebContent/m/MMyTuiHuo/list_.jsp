      ...
      <c:forEach items="${entityList}" var="cur">
      	<li data-id="${cur.id}"  id="order_state_${cur.id}">
	          ...
	          <div class="info">
	            ...
	            <div class="ordermain">
	              <p>下单时间：<em><fmt:formatDate value="${cur.add_date}" pattern="yyyy-MM-dd HH:mm:ss" /></em></p>
	            </div>
	            <c:if test="${cur.audit_state eq 1 or cur.audit_state eq 2}">
	            <div class="ordermain">
	              <p>审核时间：<em><fmt:formatDate value="${cur.audit_date}" pattern="yyyy-MM-dd HH:mm:ss" /></em></p>
	            </div>
	            </c:if>
	            <div class="ordermain">
	              <p>总价：<em>¥<fmt:formatNumber value="${cur.price}" pattern="0.##"/></em></p>
	              <p>审核状态：<em><c:choose>
	            ...
	            <div class="ordermain">
	              <p>退货方式：<em><c:choose>
              				<c:when test="${cur.expect_return_way eq 1}">退货退款</c:when>
              				<c:when test="${cur.expect_return_way eq 2}">换货</c:when>
              				<c:when test="${cur.expect_return_way eq 3}">仅退款</c:when>
            				</c:choose></em></p>
	              <p>退货原因：<em>${cur.map.returnTypeName}</em></p>
	            </div>
	            <c:url var="url" value="MMyTuiHuo.do?method=view&id=${cur.id}&order_type=${af.map.order_type}" />
           		<div class="orderedit"> <span></span> <span> <a href="javascript:goUrl('${url}');" class="buy">查看</a> </span> </div>
	          </div>
	        </div>
<%-- 	        <a href="${ctx}/m/MMyOrderDetail.do?mehod=view&order_id=${cur.id}&from=user" class="btn-combg"></a>  --%>
      	</li>
      </c:forEach>
    </ul>
  </div>
