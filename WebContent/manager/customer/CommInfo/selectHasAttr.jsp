    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="1" class="tableClassSearch">
      <tr>
        <td>规格名称：
            ...
            &nbsp;
            <button class="bgButtonFontAwesome" type="button" id="btn_submit"><i class="fa fa-plus-square"></i>确定所选</button>
          </td>
      </tr>
    </table>
  </html-el:form>

 <html-el:form action="/customer/CommInfo" enctype="multipart/form-data" styleClass="selectAttrForm">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="backTable" align="left">
    ...
    <tr>
      <td colspan="2" align="center">
          <button class="bgButtonFontAwesome" type="button" id="btn_submit2"><i class="fa fa-plus-square"></i>确定所选</button>
      </td>
    </tr>
  </table>
  </html-el:form>
	...
	if (checkedCount == 0) {
		...
		$.ajax({
			...
			success: function(data) {
				if(data.code == 1){
					W.parentGoUrl('${ctx}/manager/customer/CommInfo.do?method=listattr&comm_id=${af.map.comm_id}');
					api.close();  
				}else{
					alert(data.msg);
				}
