...
<div align="center" style="height:550px;overflow:auto;">
    <fieldset>
        ...
        <div id="chZj" style="height:45px;overflow:auto;">
            <ul id="fdul" class="fdul">
                <c:forEach items="${commList}" var="cus">
    ...
    <div>
        <html-el:form action="/customer/CommWelfareApply.do">
    ...
    <form id="listForm" name="listForm" method="post" action="Coupons.do?method=delete">
        <div>
            <table width="100%" cellspacing="0" cellpadding="0" class="tableClass" align="left" id="tableForList">
               ...
               <c:if test="${not empty cur.map.choose}">
                    <tr align="center" id="forTrClick" style="cursor:pointer;" title="点击选择" class = "cked" >
                        <td >
                        	<input name="pks" type="checkbox" checked="checked" data-flag="1" id="pks_${cur.id}" value="${cur.id}+${cur.comm_name}+${cur.map.price}" class="cked"/>
                        </td>
                        <td align="left"><c:out value="${fnx:abbreviate(cur.comm_name, 60, '...')}" />
                        </td>
                        <td><fmt:formatNumber pattern="#0.##" value="${cur.map.price}"/></td>
                    </tr>
               </c:if>
               <c:if test="${empty cur.map.choose}">
                    <tr align="center" id="forTrClick" style="cursor:pointer;" title="点击选择" >
                        <td >
                        	<input name="pks" type="checkbox" data-flag="0" id="pks_${cur.id}" value="${cur.id}+${cur.comm_name}+${cur.map.price}" class="cked"/>
                        </td>
                        <td align="left"><c:out value="${fnx:abbreviate(cur.comm_name, 60, '...')}" />
                        </td>
                        <td><fmt:formatNumber pattern="#0.##" value="${cur.map.price}"/></td>
                ...
                <tr>
                    <td><script type="text/javascript" src="${ctx}/commons/scripts/pager.js">;</script>
                        <script type="text/javascript">
                            ...
                            pager.addHiddenInputs("discount_comm_ids", "${af.map.discount_comm_ids}");
                            pager.addHiddenInputs("discount_comm_names", "${af.map.discount_comm_names}");
                            pager.addHiddenInputs("discount_comm_prices", "${af.map.discount_comm_prices}");
...
function returnVal(){
	var api = frameElement.api, W = api.opener;
	W.returnVal($("input[name='discount_comm_ids']").val(),$("input[name='discount_comm_names']").val(),$("input[name='discount_comm_prices']").val());
	api.close();
}
//]]></script>
</body>
