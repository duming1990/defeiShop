...
<div class="divContent">
  ...
  <html-el:form action="/customer/CustomerSuperMarketCommInfo" styleClass="searchForm">
  ...
  <form id="listForm" name="listForm" method="post" action="CustomerSuperMarketCommInfo.do?method=delete">
...
<%--             <a href="${url}" title="${fn:escapeXml(cur.comm_name)}"> --%>
          ...
          <td><c:choose>
              <c:when test="${cur.is_aid eq 0}"><span class="tip-danger">否</span></c:when>
              <c:when test="${cur.is_aid eq 1}"><span class="tip-success">是</span></c:when>
            </c:choose>
            </td>
          <td>${fn:escapeXml(cur.map.set_poors)}</td>
          <td><c:if test="${cur.is_del eq 0}"> <a class="butbase" id="edit" onclick="confirmUpdate('null', 'CustomerSuperMarketCommInfo.do', 'id=${cur.id}&mod_id=${af.map.mod_id}&' + $('#bottomPageForm').serialize())"><span id="${cur.id}" class="icon-edit">修改商品</span></a><br/>
            </c:if>
          </td>
        </tr>
  ...
  <div class="black">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="CustomerSuperMarketCommInfo.do">
...
$(document).ready(function(){
	...
	$("#downloadQrcode").click(function(){
		var submit = function (v, h, f) {
		    if (v == true) {
		    	location.href = "${ctx}/manager/customer/CustomerSuperMarketCommInfo.do?method=downloadQrcode&" + $('.searchForm').serialize();
		    }
		    return true;
		};
		var tip = "确定导出二维码图片吗？";
	...
	$("#btn_submit").click(function(){
	 ...
	 $("#download").click(function(){
			location.href = "CustomerSuperMarketCommInfo.do?method=toExcelForSuperMarketCommInfo&" + $(".searchForm").serialize();
		});
