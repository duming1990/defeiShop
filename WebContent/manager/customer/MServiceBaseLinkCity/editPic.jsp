...
<div class="divContent">
  ...
  <html-el:form action="/customer/MServiceBaseLinkCity" enctype="multipart/form-data">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      ...
      <tr>
        ...
        <div class="files-warp" id="image_path_warp">
          ...
          <div style="color:rgb(241, 42, 34); padding: 5px;"> 
              <c:if test="${af.map.link_type eq 10}"> [图片尺寸] 建议：[798px * 380px] </c:if>
              <c:if test="${af.map.link_type eq 20}"> [图片尺寸] 建议：[798px * 170px] </c:if>
              <c:if test="${(af.map.pre_number eq 1) and (af.map.par_son_type eq 1)}">[图片尺寸] 建议：[798px * 285px] </c:if>
              <c:if test="${(af.map.pre_number eq 2) and (af.map.par_son_type eq 1)}">[图片尺寸] 建议：[370px  * 390px] </c:if>
              <c:if test="${(af.map.pre_number eq 2) and (af.map.par_son_type eq 2)}">[图片尺寸] 建议：[370px  * 195px] </c:if>
              <c:if test="${(af.map.pre_number eq 2) and (af.map.par_son_type eq 3)}">[图片尺寸] 建议：[770px * 250px] </c:if>
              <c:if test="${(af.map.pre_number eq 3) and (af.map.par_son_type eq 1)}">[图片尺寸] 建议：[245px  * 330px] </c:if>
              <c:if test="${(af.map.pre_number eq 4) and (af.map.par_son_type eq 1)}">[图片尺寸] 建议：[370px  * 470px] </c:if>
              <c:if test="${(af.map.pre_number eq 5) and (af.map.par_son_type eq 1)}">[图片尺寸] 建议：[390px  * 490px] </c:if>
         </div>
         </td>
      </tr>
     <c:if test="${af.map.link_type eq 20}">
        <tr>
      	...
      	<img src="${ctx}/styles/mserviceIndex/img/style1.jpg" height="150" class="selectImg" data-flag="1"/>
      	<img src="${ctx}/styles/mserviceIndex/img/style2.jpg" height="150" class="selectImg" data-flag="2"/>
      	<img src="${ctx}/styles/mserviceIndex/img/style3.jpg" height="150" class="selectImg" data-flag="3"/>
      	<img src="${ctx}/styles/mserviceIndex/img/style4.jpg" height="150" class="selectImg" data-flag="4"/>
      	<img src="${ctx}/styles/mserviceIndex/img/style5.jpg" height="150" class="selectImg" data-flag="5"/>
      	</td>
      </tr> </c:if>
      <tr>
...
$(document).ready(function(){
	...
	$("#btn_submit").click(function(){
	 ...
	 if(!$("#isNotStyle").val()){//假如是楼层，不给修改样式
