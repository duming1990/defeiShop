...
<div class="divContent">
  ...
  <html-el:form action="/customer/EntpBaseLink">
  ...
  <form id="listForm" name="listForm" method="post" action="EntpBaseLink.do?method=delete">
    <div style="padding-bottom:5px;">
      ...
      <input type="button" name="add" id="add" class="bgButton" value="添 加" onclick="location.href='EntpBaseLink.do?method=add&mod_id=${af.map.mod_id}&type=${af.map.type}&link_id=${link_id}&link_type=${af.map.link_type}&pre_number=${af.map.pre_number}&par_id=${af.map.par_id}&par_son_type=${af.map.par_son_type}';" />
        ...
        <tr align="center">
          ...
          <td nowrap="nowrap">
         	<c:if test="${cur.link_type ne 20}">
           		 <a class="butbase" id="edit" onclick="confirmUpdate(null, 'EntpBaseLink.do', 'id=${cur.id}&type=${af.map.type}&link_id=${link_id}&' + $('#bottomPageForm').serialize())"><span class="icon-edit">修改</span></a>
            </c:if>
         	<c:if test="${cur.link_type eq 20}">
           		 <a class="butbase" id="edit" onclick="confirmUpdate(null, 'EntpBaseLink.do', 'id=${cur.id}&isNotStyle=true&type=${af.map.type}&link_id=${link_id}&' + $('#bottomPageForm').serialize())"><span class="icon-edit">修改</span></a>
            </c:if>
          	<c:if test="${cur.is_del eq 0}">
          	    <a class="butbase" id="remove" onclick="confirmDelete(null, 'EntpBaseLink.do', 'id=${cur.id}&link_id=${link_id}&' + $('#bottomPageForm').serialize())"><span class="icon-remove">删除</span></a>
            </c:if>
  ...
  <div class="pageClass">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="EntpBaseLink.do">
