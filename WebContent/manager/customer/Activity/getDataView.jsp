...
<%--           <td align="center"><fmt:formatDate value="${cur.add_date}" pattern="yyyy-MM-dd HH:mm:ss" /></td> --%>
            <td nowrap="nowrap">
            <a onclick="getDataViewComm(${userInfo.own_entp_id},${cur.activity_id})">
            	商品销售排名
            </a>
            </br>
            <a onclick="getDataViewOrder(${userInfo.own_entp_id},${cur.activity_id})">
            	订单交易明细
            </a>
              </td>
        </tr>
...
function getDataViewComm(entp_id,activity_id){
	var url = "${ctx}/manager/customer/Activity.do?method=getDataViewComm&entp_id="+entp_id+"&activity_id="+activity_id;
	$.dialog({
		title:  "商品销售排名",
		width:  850,
		height: 500,
...
function getDataViewOrder(id,activity_id){
	var url = "${ctx}/manager/customer/OrderQuery.do?activity=true&order_type=90&order_state=50&entp_id="+id+"&activity_id="+activity_id;
	$.dialog({
		title:  "商品销售排名",
		width:  1000,
		height: 500,
