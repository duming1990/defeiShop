      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        <tr align="center">
          ...
          <a href="Activity.do?method=view&amp;id=${cur.id}">
            ...
            <c:if test="${empty admin }">
            <a  onclick="getDataViewOrder(${ui.id})">
            <c:out value="${fnx:abbreviate(cur.title, 50, '...')}" />
            </a>
            </c:if>
            </td>
          ...
          <td>
            ...
            <span class="icon-ok">审核</span></a>
              ...
              <logic-el:match name="popedom" value="+4+">
                ...
                <c:if test="${empty admin }">
	                <c:set var="apply_name" value="申请" />
	                <c:if test="${cur.map.audit_state eq -1 }">
	                <c:set var="apply_name" value="再次申请" />
	                </c:if>
	                <c:if test="${cur.map.audit_state ne 0}">
	                	<a class="butbase" onclick="apply(${cur.id})"><span class="icon-edit">${apply_name}</span></a>
	                	</c:if>
	                	<c:if test="${cur.map.audit_state eq 0}">
...
function getDataViewOrder(id){
	var url = "${ctx}/manager/admin/OrderQuery.do?order_type=90&entp_id="+id;
	$.dialog({
		title:  "商品销售排名",
		width:  1000,
		height: 800,
...
function apply(id) {
	...
	var url = "${ctx}/manager/customer/ActivityApply.do?method=chooseCommInfo&activity_id="+id+"&t=" + new Date().getTime();
	$.dialog({
		title:  "选择商品",
		width:  800,
		height: 800,
