...
<div class="mainbox mine">
  ...
  <html-el:form action="/customer/VillageTour">
        ...
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" align="left">
            <tr>
              <td width="6%" nowrap="nowrap">标题：
                <html-el:text property="tour_name_like" styleId="tour_name_like" style="width:140px;" styleClass="webinput"/>
                 &nbsp;添加时间 从：
  ...
  <form id="listForm" name="listForm" method="post" action="VillageTour.do?method=delete">
    <div style="padding:5px;">
    <logic-el:match name="popedom" value="+1+">
      <button class="bgButtonFontAwesome" type="button" onclick="location.href='VillageTour.do?method=add&par_id=${af.map.par_id}&mod_id=${af.map.mod_id}';" ><i class="fa fa-plus-square"></i>添加</button>
    </logic-el:match>
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        <tr>
         <td align="center">
            ...
            <td align="center">${fn:escapeXml(cur.tour_name)}</td>
          <td align="center">
            <c:if test="${not empty cur.tour_logo}">
          	<c:set var="img" value="${ctx}/${cur.tour_logo}@s400x400" />
        	</c:if>
        	<c:if test="${fn:contains(cur.tour_logo, 'http://')}">
		 	 <c:set var="img" value="${cur.tour_logo}"/>
		 	</c:if>
		 	<a href="${img}" target="blank"><img src="${img}" height="100"/></a>
		  </td>
<%--           <td align="center">${cur.tour_content}</td> --%>
          <td align="center">${cur.map.full_name}</td>
          <td align="center"><fmt:formatDate value="${cur.tour_start_date}" pattern="yyyy-MM-dd" /></td>
          <td align="center"><fmt:formatDate value="${cur.tour_end_date}" pattern="yyyy-MM-dd" /></td>
          <td align="center"><fmt:formatDate value="${cur.add_date}" pattern="yyyy-MM-dd" /></td>
          <td align="center" nowrap="nowrap">
            <div style="padding-top: 2px;">
              <logic-el:match name="popedom" value="+2+">
			    <a class="butbase" onclick="confirmUpdate(null, 'VillageTour.do', 'id=${cur.id}&mod_id=${af.map.mod_id}&' + $('#bottomPageForm').serialize())"><span class="icon-edit">修改</span></a>
              </logic-el:match>
              <logic-el:notMatch name="popedom" value="+2+"><a class="butbase but-disabled"><span class="icon-edit">修改</span></a> </logic-el:notMatch>
              <logic-el:match name="popedom" value="+4+">
                <a class="butbase" onclick="confirmDelete(null, 'VillageTour.do', 'id=${cur.id}&mod_id=${af.map.mod_id}&' + $('#bottomPageForm').serialize())"><span class="icon-remove">删除</span></a>
              </logic-el:match>
  ...
  <div class="black">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="VillageTour.do">
      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" >
        <tr>
          <td height="10" ><script type="text/javascript" src="${ctx}/commons/scripts/pager.js">;</script>
            ...
            pager.addHiddenInputs("tour_name_like", "${fn:escapeXml(af.map.tour_name_like)}");
