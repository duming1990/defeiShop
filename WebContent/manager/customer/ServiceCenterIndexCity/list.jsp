...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		<style type="">
			.r12r1 div:nth-child(1) img { width: 49%; margin-top: 25px; float: left; } .r12r1 div:nth-child(2) img { width: 49%; margin-top: 25px; float: right; } .r12r1 div:nth-child(3) img { width: 49%; margin-top: 25px; float: right; } .r12r1 div:nth-child(4) img { width: 100%; margin-top: 25px; float: right; } .preview{ width:50px; height:40px; border-radius:20px; border-style:none; background:#ec6941; font-size:14px; color:rgba(249,248,248,1.00); position: absolute; left: -120px; top: 150px; } .areasbg{ width:150px; height:40px; position: absolute; left: -160px; top: 300px; } .beautybg1{ box-shadow: inset 0px 1px 0px 0px #a4e271; background: -webkit-gradient(linear, left top, left bottom, color-stop(0.05, #89c403), color-stop(1, #77a809)); background: -moz-linear-gradient(center top, #89c403 5%, #77a809 100%); filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#89c403',endColorstr='#77a809'); background-color: #89c403; -moz-border-radius: 5px; -webkit-border-radius: 5px; border-radius: 5px; border: 1px solid #74b807; display: inline-block; color: #ffffff; font-family: arial; font-size: 12px; font-weight: bold; padding: 4px 10px; text-decoration: none; text-shadow: 1px 1px 0px #528009; line-height: 15px; cursor: pointer; }
		</style>
	</head>
	...
	<body style="${bodyBackgroudImage}">
			...
			<div class="banner">
				<div class="swiper-container">
					<div class="areasSmall" style="left:400px;top:100px;">
						<a href="${ctx}/manager/customer/ServiceBaseLinkCity.do?mod_id=${af.map.mod_id}&link_type=10&main_type=20&link_id=${entity.id}" class="beautybg">编辑轮播图</a>
					</div>
			...
			<div class="container">
				<div class="guan">
					<div class="swiper-container swiperguan">
						<div class="swiper-wrapper" style="height: 306px">
							<c:forEach var="cur" items="${baseLink30List}">
								<c:if test="${not empty cur.image_path}">
									<a href="${ctx}/IndexTsg.do?method=index&Link_id=${cur.link_id}&" target="_blank"><img src="${ctx}/${cur.image_path}" /></a>
								</c:if>
							</c:forEach>
						</div>
					</div>
				...
				<div class="areasbg">
					<a href="${ctx}/manager/customer/ServiceBaseLinkCity.do?method=indexTsg&mod_id=${af.map.mod_id}&link_type=30&main_type=20&link_id=${entity.id}&p_index=${entity.p_index}&" class="beautybg">编辑背景图</a>
				</div>
				...
				<button class="preview"><a href="${ctx}/IndexTsg.do?method=index&p_index=${entity.p_index}" style="color: white;text-decoration:none;" target="_blank">预览</a></button>
				<c:forEach var="cur" items="${baseLink20List}" varStatus="vs">
					<c:if test="${cur.pre_number eq 1}">
						<div class="lanmuhe">
							<img src="${ctx}/${cur.image_path}" title="${cur.title}" />
						</div>
						<div class="areas" style="position: absolute;left:5px">
							<a href="${ctx}/manager/customer/ServiceBaseLinkCity.do?mod_id=${af.map.mod_id}&link_type=${vs.count}01&main_type=${cur.main_type}&link_id=${cur.link_id}&pre_number=${cur.pre_number}&par_id=${cur.id}&par_son_type=1" class="beautybg">编辑</a>
						</div>
						<div class="r1r1">
						...
						<div class="r12r1" style="position: relative;">
							...
							<div>
								<div class="areas" style="position: absolute;right: 540px;top: 30px">
									<a href="${ctx}/manager/customer/ServiceBaseLinkCity.do?mod_id=${af.map.mod_id}&main_type=${cur.main_type}&link_type=${vs.count}02&link_id=${cur.link_id}&pre_number=${cur.pre_number}&par_id=${cur.id}&par_son_type=2" class="beautybg">编辑</a>
								</div>
							...
							<div>
								<div class="areas" style="position:absolute; top: 760px;left: 10px">
									<a href="${ctx}/manager/customer/ServiceBaseLinkCity.do?mod_id=${af.map.mod_id}&main_type=${cur.main_type}&link_type=${vs.count}03&link_id=${cur.link_id}&pre_number=${cur.pre_number}&par_id=${cur.id}&par_son_type=3" class="beautybg">编辑</a>
								</div>
		...
		<script type="text/javascript">
			...
			function editFloor(id) {
				var url = "${ctx}/manager/customer/ServiceBaseLinkCity.do?&mod_id=${af.map.mod_id}&link_type=20&main_type=20&link_id=" + id;
				$.dialog({
					title: "编辑楼层",
					width: 1400,
					height: 700,
