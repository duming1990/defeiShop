  ...
  <html-el:form action="/customer/MBaseLink" enctype="multipart/form-data">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      ...
      <tr>
        ...
        <td colspan="2" width="85%"><html-el:text property="title" styleId="title" maxlength="125" style="width:480px" styleClass="webinput" value="${af.map.title}"/>
...
<%--         	<html-el:hidden property="comm_id" /> --%>
      ...
      <tr>
        <td width="15%" class="title_item">副标题：</td>
        <td colspan="2" width="85%"><html-el:text property="pre_varchar" styleId="pre_varchar" maxlength="125" style="width:480px" styleClass="webinput" value="${af.map.pre_varchar}"/></td>
      </tr>   
      <tr>
        ...
        <div class="files-warp" id="image_path_warp">
          ...
          <c:if test="${af.map.link_type eq 200}">[图片尺寸] 建议：[465px * 243px] </c:if>
      ...
      <tr>
        ...
        <td colspan="2" width="85%"><html-el:text property="link_url" styleId="link_url" maxlength="200" style="width:50%" styleClass="webinput" value="${af.map.link_url}"/>&nbsp;请用"http://"或者"https://"开头</td>
      </tr>
      <tr>
        <td width="15%" class="title_item"><span style="color: #F00;">*</span>产品价格：</td>
        <td colspan="2" width="85%"><html-el:text property="pd_price" styleId="pd_price" maxlength="12" styleClass="webinput" value="${af.map.pd_price}"/>&nbsp;元</td>
      </tr>
      <tr>
        <td width="15%" class="title_item" nowrap="nowrap">排序值：</td>
        <td colspan="2" width="85%"><html-el:text property="order_value" styleId="order_value" maxlength="4" size="4" styleClass="webinput" value="${af.map.order_value}"/>
...
function getCommInfoList() {
	var url = "${ctx}/BaseCsAjax.do?method=chooseCommInfo&dir=admin&is_p_index=true&is_jd=0&comm_type_in=2&ajax=getCommInfo&t=" + new Date().getTime();
	$.dialog({
		title:  "选择商品",
		width:  760,
		height: 360,
...
function getCommLinkInfo(comm_id){
	if(null != comm_id && '' != comm_id){
		 $.ajax({
			   type: "POST",
			   url: "${ctx}/manager/customer/MBaseLink.do?method=getCommLinkInfo",
