...
<div class="divContent">
  ...
  <html-el:form action="/customer/ServiceBaseLink">
  ...
  <form id="listForm" name="listForm" method="post" action="ServiceBaseLink.do?method=delete">
    <div style="padding-bottom:5px;">
      ...
      <input type="button" name="add" id="add" class="bgButton" value="添 加" onclick="location.href='ServiceBaseLink.do?method=add&mod_id=${af.map.mod_id}&type=${af.map.type}&link_id=${link_id}&link_type=${af.map.link_type}&pre_number=${af.map.pre_number}&par_id=${af.map.par_id}&par_son_type=${af.map.par_son_type}';" />
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        <tr align="center">
          ...
          <td align="left">
           <a href="${ctx}/manager/customer/ServiceBaseLink.do?method=view&amp;id=${cur.id}&mod_id=${af.map.mod_id}">
          ...
          <td nowrap="nowrap">
            ...
            <a class="butbase" id="edit" onclick="confirmUpdate(null, 'ServiceBaseLink.do', 'id=${cur.id}&type=${af.map.type}&link_id=${link_id}&' + $('#bottomPageForm').serialize())"><span class="icon-edit">修改</span></a>
            </c:if>
            ...
            <a class="butbase" id="edit" onclick="confirmUpdate(null, 'ServiceBaseLinkCity.do', 'id=${cur.id}&isNotStyle=true&type=${af.map.type}&link_id=${link_id}&main_type=${main_type}&' + $('#bottomPageForm').serialize())"><span class="icon-edit">修改</span></a>
          	...
          	<c:if test="${cur.is_del eq 0}">
          	    <a class="butbase" id="remove" onclick="confirmDelete(null, 'ServiceBaseLink.do', 'id=${cur.id}&link_id=${link_id}&' + $('#bottomPageForm').serialize())"><span class="icon-remove">删除</span></a>
            </c:if>
  ...
  <div class="pageClass">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="ServiceBaseLink.do">
