...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="${ctx}/styles/mserviceIndex/css/index.css?20180616" />
		...
		<body>
			<div class="container index" style="width: 50% ;position: relative;float: left;left: 25%">
				<div class="swiper-container">
					<div class="areasSmall">
						<a href="${ctx}/manager/customer/MServiceBaseLinkCity.do?mod_id=${af.map.mod_id}&link_type=10&main_type=20&link_id=${entity.id}" class="beautybg">编辑轮播图</a>
					</div>
							...
							<c:forEach var="cur" items="${baseLink30List}">
								<c:if test="${(not empty cur.image_path) and (cur.main_type eq 10)}">
									<div class="swiper-slide imgs ">
										<a href="${ctx}/m/IndexMTsg.do?method=index&link_id=${cur.link_id}" title="${cur.title}" target="_blank"><img src="${ctx}/${cur.image_path}" class="img" /></a>
									</div>
								</c:if>
							</c:forEach>
						</div>
				...
				<div class="areasbg">
					<a href="${ctx}/manager/customer/MServiceBaseLinkCity.do?method=indexTsg&mod_id=${af.map.mod_id}&link_type=30&main_type=20&link_id=${entity.id}&p_index=${entity.p_index}&" class="beautybg">编辑背景图</a>
				</div>
				...
				<c:forEach var="cur" items="${baseLink20List}" varStatus="vs">
					<c:if test="${cur.pre_number eq 1}">
						<div class="div2">
							...
							<div class="areas" style="position: absolute;">
								<a href="${ctx}/manager/customer/MServiceBaseLinkCity.do?mod_id=${af.map.mod_id}&link_type=${vs.count}01&link_id=${cur.link_id}&main_type=${cur.main_type}&pre_number=${cur.pre_number}&par_id=${cur.id}&par_son_type=1" class="beautybg">编辑</a>
							</div>
					...
					<c:if test="${cur.pre_number eq 2}">
						...
						<div class="div3">
							...
							<div class="right">
								<div class="areas" style="position: absolute;">
									<a href="${ctx}/manager/customer/MServiceBaseLinkCity.do?mod_id=${af.map.mod_id}&link_type=${vs.count}02&link_id=${cur.link_id}&main_type=${cur.main_type}&pre_number=${cur.pre_number}&par_id=${cur.id}&par_son_type=2" class="beautybg">编辑</a>
								</div>
						...
						<div class="div2">
							<div class="areas" style="position: absolute; left: 9px">
								<a href="${ctx}/manager/customer/MServiceBaseLinkCity.do?mod_id=${af.map.mod_id}&link_type=${vs.count}03&link_id=${cur.link_id}&main_type=${cur.main_type}&pre_number=${cur.pre_number}&par_id=${cur.id}&par_son_type=3" class="beautybg">编辑</a>
							</div>
			...
			<script type="text/javascript">
				...
				function editFloor(id) {
					var url = "${ctx}/manager/customer/MServiceBaseLinkCity.do?mod_id=${af.map.mod_id}&link_type=20&main_type=20&link_id=" + id;
					$.dialog({
						title: "编辑楼层",
						width: 1400,
						height: 700,
