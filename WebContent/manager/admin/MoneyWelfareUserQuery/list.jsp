...
<div style="width: 99%" class="divContent">
  ...
  <html-el:form action="/admin/MoneyWelfareUserQuery" styleClass="searchForm">
  ...
  <form id="listForm" name="listForm" method="post" action="MoneyWelfareUserQuery.do?method=delete">
...
<%--           <fmt:formatNumber pattern="#0.00" value="${cur.map.has_tixian_money}" />元</td> --%>
          ...
          <c:if test="${isContains}" >
            <td align="center"><c:if test="${cur.info_state eq 0}">
                <logic-el:match name="popedom" value="+8+"> <a class="butbase" onclick="doNeedMethod(null, 'MoneyWelfareUserQuery.do', 'audit','mod_id=${af.map.mod_id}&id=${cur.id}&'+$('#bottomPageForm').serialize())"><span class="icon-ok">审核</span></a> </logic-el:match>
...
<!--           <td>&nbsp;</td> -->
  ...
  <div class="pageClass">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="MoneyWelfareUserQuery.do">
...
$("#download").click(function(){
		...
		var submit = function (v, h, f) {
		    if (v == true) {
		    	location.href = "${ctx}/manager/admin/MoneyWelfareUserQuery.do?method=toExcel&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
		    } else {
		    	location.href = "${ctx}/manager/admin/MoneyWelfareUserQuery.do?method=toExcel&code=GBK&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
		    }
		    return true;
		};
		...
		$.jBox.alert("请至少选择一个审核项！", '提示');
			...
			var submit = function (v, h, f) {
			   ...
			   $.jBox.tip("执行中...", 'loading');
			     ...
			     $.post("MoneyWelfareUserQuery.do?method=auditAll&info_state="+info_state + "&audit_memo=" + audit_memo + "&" + $(form).serialize() ,{},function(data){
	...
	if (checkedCount == 0) {
		...
		var submit = function (v, h, f) {
		    if (v == true) {
		     $.jBox.tip("执行中...", 'loading');
		     window.setTimeout(function () { 
		     $.post("MoneyWelfareUserQuery.do?method=payAll"+"&" + $(form).serialize(),function(data){
		     //$.jBox.tip(data, "success");
		     location.reload();
		     });
		     }, 1000);
...
function confirmAudit(id){
		$.dialog({
			...
			height: 124,
	        lock:true ,
			content:"url:${ctx}/manager/admin/MoneyWelfareUserQuery.do?method=payForm&id=" +id + "&" + $('#bottomPageForm').serialize()
		});
}
...
function confirmTk(id){
	var submit = function (v, h, f) {
	    if (v == true) {
	    	 ...
	    	 $.jBox.tip("执行中...", 'loading');
		     window.setTimeout(function () { 
		    	 location.href = "${ctx}/manager/admin/MoneyWelfareUserQuery.do?method=confirmTk&id=" + id+"&mod_id=${af.map.mod_id}" + "&tuikuan_memo=" + tuikuan_memo;
		     }, 1000);
	    }
	    return true;
	};
