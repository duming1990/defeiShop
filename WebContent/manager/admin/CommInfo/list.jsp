      ...
      <tr>
        <td>产品类别：
          ...
          <html-el:text property="comm_name_like" styleClass="webinput" maxlength="50" style="width:200px;"/>
            ...
            &nbsp;上下架状态：
            <html-el:select property="is_sell" >
              <html-el:option value="">全部</html-el:option>
              <html-el:option value="1">已上架</html-el:option>
              <html-el:option value="0">已下架</html-el:option>
            </html-el:select>
            &nbsp;下架时间 从：
                <html-el:text property="st_down_date" styleId="st_down_date" size="10" maxlength="10" readonly="true" styleClass="webinput" onclick="WdatePicker()" />
            至
            <html-el:text property="en_down_date" styleId="en_down_date" size="10" maxlength="10" readonly="true" styleClass="webinput" onclick="WdatePicker()" />
            &nbsp;
  ...
  <form id="listForm" name="listForm" method="post" action="CommInfo.do?method=delete">
    <div style="text-align: left;padding: 5px 0px;">
       ...
       <logic-el:match name="popedom" value="+1+">  
         <input type="button" name="add" id="add" class="bgButton" value="添 加" onclick="location.href='CommInfo.do?method=add&mod_id=${af.map.mod_id}&comm_type=${af.map.comm_type}';" />
         <input type="button" name="add" id="add" class="bgButton" value="变更产品类别" onclick="location.href='CommInfo.do?method=editBaseClass&mod_id=${af.map.mod_id}&comm_type=${af.map.comm_type}';" />
       </logic-el:match>
    </div>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      <tr class="tite2">
        <th width="2%" ><input name="chkAll" type="checkbox" id="chkAll" value="-1" onclick="checkAll(this);" /></th>
        <th nowrap="nowrap">商品名称</th>
        <th width="8%">商品主图</th>
<!--         <th width="12%">商品发布信息</th> -->
        <th width="5%">产品类别</th>
        <th width="6%">商品编号</th>
<!--         <th width="9%">商品二维码</th> -->
        <th width="6%">销售价格</th>
        <th width="8%">上下架时间 </th>
<!--         <th width="5%">是否提供发票/自提</th> -->
        <th width="5%">商品库存</th>
        <th width="6%">审核状态</th>
        <th width="6%" >审核操作</th>
        <th width="6%" >操作</th>
...
<%--             </c:if> --%>
            ...
            <td><img src="${ctx}/${cur.main_pic}" width="120" /></td>

<!--           <td  align="left"> -->
<%-- 			发布用户:${fn:escapeXml(cur.map.user_name)}<br/> --%>
<%-- 			真实姓名:(${fn:escapeXml(cur.map.real_name)})<br/> --%>
<%-- 		          所属区域:${fn:escapeXml(cur.map.full_name)}<br/> --%>
<%-- 		          联系电话:${fn:escapeXml(cur.map.mobile)}<br/> --%>
<%-- 		         企业名称:${fn:escapeXml(cur.map.entp_name)} --%>
...
<%--            <img src="${ctx}/${cur.comm_qrcode_path}" width="100%"/> --%>
<%--             </c:if> --%>
<!--           </td> -->
...
<!--           </td> -->
          ...
          <logic-el:match name="popedom" value="+8+"> 
            <a class="butbase" onclick="doNeedMethod(null, 'CommInfo.do', 'audit','id=${cur.id}&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize())"><span class="icon-ok">审核</span></a><br/>
          </logic-el:match>
            <a class="butbase" onclick="updateSaleCount('${cur.id}')"><span class="icon-ok">销量</span></a>
            <a class="butbase" onclick="pingjia('${cur.id}')"><span class="icon-node">评价</span></a>
            <div style="padding: 2px;"><span class="qtip tip-danger" title="排序值：${cur.order_value}，排序值大的靠前显示。">排序:${cur.order_value}</span></div></td>
          <td><c:if test="${cur.is_del eq 0}"> <a class="butbase" id="edit" onclick="confirmUpdate('null', 'CommInfo.do', 'id=${cur.id}&mod_id=${af.map.mod_id}&' + $('#bottomPageForm').serialize())"><span id="${cur.id}" class="icon-edit">修改商品</span></a><br/>
              <a class="butbase" id="edittcfw" onclick="doNeedMethod('null', 'CommInfo.do', 'edittcfw', 'comm_id=${cur.id}&' + $('#bottomPageForm').serialize())"><span id="${cur.id}" class="icon-edit">修改套餐</span></a><br/>
              <c:if test="${cur.audit_state ne 1}"><a class="butbase" id="remove" onclick="confirmDelete(null, 'CommInfo.do', 'id=${cur.id}&cls_id=${cur.cls_id}&mod_id=${af.map.mod_id}&' + $('#bottomPageForm').serialize())"><span id="${cur.id}" class="icon-remove">删除商品</span></a></c:if>
            ...
            <c:if test="${cur.is_del eq 1}"> <a class="butbase" id="return" onclick="confirmNeedMethod('确认恢复数据？', 'CommInfo.do', 'hf', 'id=${cur.id}&mod_id=${af.map.mod_id}&' + $('#bottomPageForm').serialize())"><span id="${cur.id}" class="icon-return">恢复商品</span></a> </c:if>
          </td>
        </tr>
        ...
        <tr>
          <td><script type="text/javascript" src="${ctx}/commons/scripts/pager.js">;</script>
            <script type="text/javascript">
					...
					pager.addHiddenInputs("st_down_date", "${af.map.st_down_date}");
					pager.addHiddenInputs("en_down_date", "${af.map.en_down_date}");
					pager.addHiddenInputs("is_sell", "${af.map.is_sell}");
...
$(document).ready(function(){
	...
	$("#downloadQrcode").click(function(){
		var submit = function (v, h, f) {
		    if (v == true) {
		    	location.href = "${ctx}/manager/admin/CommInfo.do?method=downloadQrcode&" + $('.searchForm').serialize();
		    }
		    return true;
		};
		var tip = "确定导出二维码图片吗？";
