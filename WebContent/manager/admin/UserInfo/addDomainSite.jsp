...
<body>
     <div>
      <div class="nav_xinping">
       <html-el:form action="/admin/UserInfo.do" styleClass="formSavePayInfo">
       ...
       <td width="88%">
		  <html-el:text property="domain_site" maxlength="120" styleId="domain_site" style="width:200px" value="${userInfo.domain_site}"/>
		  &nbsp;&nbsp;&nbsp;<div>示例：如何设置 <span style="color:red">合肥市级馆 hf.9tiaofu.com</span>的域名？请在上方输入框内输入 <span style="color:red">hf</span>  即可！</div>
       </td>
       </tr>
       <tr>
 ...
 $("#domain_site").attr("dataType", "Require").attr("msg", "请输入二级域名");
	...
	if(Validator.Validate(f,1)){
		...
		window.setTimeout(function () { 
			$.ajax({
				type: "POST",
				url: "UserInfo.do?method=saveDomainSite",
	...
	$("#clean_submit").click(function(){
		window.setTimeout(function () { 
			$.ajax({
				type: "POST",
				url: "UserInfo.do?method=delDomainSite",
				data: $('.formSavePayInfo').serialize(),
				dataType: "json",
				error: function(request, settings) {$.jBox.tip("数据请求失败", "error");},
				success: function(data) {
