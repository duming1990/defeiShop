...
<div style="width: 99%" class="divContent">
  ...
  <html-el:form action="/admin/Msg">
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="1" class="tableClassSearch">
      <tr>
        <td>标题：
          <html-el:text property="msg_title_like" maxlength="40" style="width:150px;" styleClass="webinput" />
          &nbsp;留言状态：
  ...
  <div style="text-align: left; padding: 5px;">
    ...
    <input type="button" name="add" id="add" class="bgButton" value="写信" onclick="location.href='Msg.do?method=add&mod_id=${af.map.mod_id}';" />
  </div>
      ...
      <c:forEach var="cur" items="${entityList}">
        <tr>
          ...
          <td align="center"><c:choose>
              <c:when test="${cur.info_state eq 1}"><span class="label label-danger">已发送</span></c:when>
              <c:when test="${cur.info_state eq 2}"><span class="label label-danger">已删除</span></c:when>
              <c:otherwise>-</c:otherwise>
            </c:choose></td>
          <td align="center"><fmt:formatDate value="${cur.send_time}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
          <td align="center"><span style="cursor: pointer;" onclick="doNeedMethod(null, 'Msg.do', 'view','mod_id=${af.map.mod_id}&id=${cur.id}')"> 查看 </span></td>
        </tr>
