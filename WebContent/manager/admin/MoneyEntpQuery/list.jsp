...
<div style="width: 99%" class="divContent">
  ...
  <html-el:form action="/admin/MoneyEntpQuery" styleClass="searchForm">
  ...
  <form id="listForm" name="listForm" method="post" action="MoneyEntpQuery.do?method=delete">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      <tr>
        ...
        <th width="8%">审核状态</th>
        <th width="8%">审核时间</th>
        <c:if test="${fn:contains(popedom, '+8+') or fn:contains(popedom, '+17+')  or fn:contains(popedom, '+18+')}" var="isContains">
      ...
      <c:forEach var="cur" items="${userMoneyApplyList}" varStatus="vs" >
        <tr>
     	...
     	商家名称：${fn:escapeXml(cur.map.apply_real_name)}<br/>
          ...
          <td align="center">${fn:escapeXml(cur.cash_count_before)}</td>
          <td align="center">${fn:escapeXml(cur.cash_count)}元</td>
<%--           <td align="center">${fn:escapeXml(cur.cash_count_lock)}</td> --%>
...
<%--           <td align="center">${fn:escapeXml(cur.map.leiji_xiaofei_money)}元</td> --%>
          ...
          <c:if test="${isContains}">
            <td align="center"><c:if test="${cur.info_state eq 0}">
                <logic-el:match name="popedom" value="+8+"> <a class="butbase" onclick="doNeedMethod(null, 'MoneyEntpQuery.do', 'audit','mod_id=${af.map.mod_id}&id=${cur.id}&'+$('#bottomPageForm').serialize())"><span class="icon-ok">审核</span></a> </logic-el:match>
                <a class="butbase but-disabled"><span style="cursor: pointer;" class="icon-undo">付款</span></a> </c:if>
              ...
              <c:if test="${cur.info_state eq 2}">
                <logic-el:match name="popedom" value="+8+"> <a class="butbase but-disabled"><span class="icon-ok">已审核</span></a> </logic-el:match>
                <logic-el:match name="popedom" value="+17+"> <a class="butbase but-disabled"><span style="cursor: pointer;" class="icon-undo">已付款</span></a> </logic-el:match>
                <logic-el:match name="popedom" value="+18+">
                <a class="butbase" onclick="confirmTk(${cur.id})"><span style="cursor: pointer;" class="icon-undo">退&nbsp;&nbsp;&nbsp; 款</span></a>
                </logic-el:match>
              </c:if>
              <c:if test="${cur.info_state eq -1}">
                <logic-el:match name="popedom" value="+8+"> <a class="butbase but-disabled"><span class="icon-ok">审核</span></a> </logic-el:match>
                <logic-el:match name="popedom" value="+17+"> <a class="butbase but-disabled"><span style="cursor: pointer;" class="icon-undo">已付款</span></a> </logic-el:match>
              </c:if>
            </td>
          </c:if>
  ...
  <div class="pageClass">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="MoneyEntpQuery.do">
...
$(document).ready(function(){
	...
	$("#download").click(function(){
		...
		var submit = function (v, h, f) {
		    if (v == true) {
		    	location.href = "${ctx}/manager/admin/MoneyEntpQuery.do?method=toExcel&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
		    } else {
		    	location.href = "${ctx}/manager/admin/MoneyEntpQuery.do?method=toExcel&code=GBK&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
		    }
		    return true;
		};
		...
		$.jBox.alert("请至少选择一个审核项！", '提示');
			...
			var submit = function (v, h, f) {
			   ...
			   $.jBox.tip("执行中...", 'loading');
			     ...
			     $.post("MoneyEntpQuery.do?method=auditAll&info_state="+info_state + "&audit_memo=" + audit_memo + "&" + $(form).serialize(),{},function(data){
	...
	if (checkedCount == 0) {
		...
		var submit = function (v, h, f) {
		    if (v == true) {
		     $.jBox.tip("执行中...", 'loading');
		     window.setTimeout(function () { 
		     $.post("MoneyEntpQuery.do?method=payAll"+"&" + $(form).serialize(),function(data){
		     //$.jBox.tip(data, "success");
		     location.reload();
		     });
		     }, 1000);
...
function confirmAudit(id){
	if(confirm("确认以及给申请人付过款？")){
		location.href="MoneyEntpQuery.do?method=pay&id=" + id+"&mod_id=${af.map.mod_id}";
	}
}
function confirmTk(id){
	var submit = function (v, h, f) {
	    if (v == true) {
	    	 ...
	    	 $.jBox.tip("执行中...", 'loading');
		     window.setTimeout(function () { 
		    	 location.href = "${ctx}/manager/admin/MoneyEntpQuery.do?method=confirmTk&id=" + id+"&mod_id=${af.map.mod_id}" + "&tuikuan_memo=" + tuikuan_memo;
		     }, 1000);
	    }
	    return true;
	};
