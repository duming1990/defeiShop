  ...
  <html-el:form action="/admin/VirtualVillage.do" enctype="multipart/form-data">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="backTable">
      <tr>
       ...
       <tr>
         <td width="14%" nowrap="nowrap" class="title_item"><span style="color: #F00;">*</span>所属合伙人：</td>
         <td>
	         <html-el:text property="service_name" readonly="true" maxlength="128" style="width:250px;" styleId="service_name" value="${ServiceInfo.servicecenter_name}"/>
	         <input type="button" value="选择合伙人" onclick="chooseService()" class="bgButton" style="margin-left: 10px;"/>
	         <input type="hidden" id="p_index_service" name="p_index_service" value="${ServiceInfo.p_index}"/>
         </td>
          <td width="14%" nowrap="nowrap" class="title_item">村站地址：</td>
          <td><html-el:text property="village_address" maxlength="50" styleId="village_address" /></td>
       </tr>
       <tr>
       ...
       <tr>
         ...
         <td width="14%" nowrap="nowrap" class="title_item"><span style="color: #F00;">*</span>服务站运营时间：</td>
         <td>
...
$("#btn_submit").click(function(){
	if(Validator.Validate(f, 3)){
		 ...
		 if(null != $("#service_operation_date").val() && null != $("#service_operation_date").val()){
			if($("#service_operation_date").val()>=$("#service_operation_date_end").val()){
				alert("村站运营结束时间必须大于开始时间");
				return false;
			}
		 }
...
$("#village_name").change(function(){
	var village_name=$(this).val();
		$.ajax({
			type: "POST" , 
			url: "${ctx}/CsAjax.do" , 
			data:"method=validVillageName&village_name=" + village_name + "&is_virtual=1",
			dataType: "json", 
	        async: true, 
...
function chooseService(){
	var url = "${ctx}/manager/admin/VirtualVillage.do?method=getServiceList&is_virtual=1";
	$.dialog({
		title:  "选择合伙人",
		width:  1500,
		height: 800,
