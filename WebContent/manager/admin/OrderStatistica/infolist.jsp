...
<div align="center" class="divContent">
  ...
  <html-el:form action="/admin/OrderStatistica">
    <html-el:hidden property="method" value="orderInfoList" />
  ...
  <c:if test="${af.map.order_state ne 20}">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
    <tr>
      <th width="10%" nowrap="nowrap">序号</th>
      <th nowrap="nowrap">企业名称</th>
      <th width="20%" nowrap="nowrap">订单数</th>
      <th width="20%" nowrap="nowrap">订单合计产品销售数量</th>
      <th width="20%" nowrap="nowrap">订单合计销售金额</th>
    </tr>
    <c:forEach var="cur" items="${entityList}" varStatus="vs">
      <tr align="center">
        <td align="center"> ${vs.count}</td>
        <td align="center"> ${fn:escapeXml(cur.map.entp_name)}</td>
        <td nowrap="nowrap"> ${cur.map.cnt}</td>
        <td nowrap="nowrap"> ${cur.map.sum_num}</td>
        <td nowrap="nowrap"> ${cur.map.sum_money}</td>
      </tr>
    </c:forEach>
  </table>
  </c:if>
   <c:if test="${af.map.order_state eq 20}">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
    <tr>
      <th width="10%" nowrap="nowrap">序号</th>
      <th nowrap="nowrap">企业名称</th>
      <th width="20%" nowrap="nowrap">订单总数</th>
      <th  nowrap="nowrap" align="center"><strong>已发货数量</strong></th>
        <th  nowrap="nowrap" align="center"><strong>已收货数量</strong></th>
      <th width="20%" nowrap="nowrap">订单合计产品销售数量</th>
      <th width="20%" nowrap="nowrap">订单合计销售金额</th>
    </tr>
    <c:forEach var="cur" items="${entityList}" varStatus="vs">
      <tr align="center">
        <td align="center"> ${vs.count}</td>
        <td align="center"> ${fn:escapeXml(cur.map.entp_name)}</td>
        <td nowrap="nowrap"> ${cur.map.cnt + cur.map.entity2.map.cnt}</td>
        <td nowrap="nowrap"> ${cur.map.cnt}</td>
        <td nowrap="nowrap"> ${cur.map.entity2.map.cnt}</td>
        <td nowrap="nowrap"> ${cur.map.sum_num + cur.map.entity2.map.sum_num}</td>
        <td nowrap="nowrap"> ${cur.map.sum_money}</td>
      </tr>
    </c:forEach>
  </table>
        ...
        <tr>
          <td height="10"><script type="text/javascript" src="${ctx}/commons/scripts/pager.js">;</script> 
            <script type="text/javascript">
					...
					pager.addHiddenInputs("method", "orderInfoList");
