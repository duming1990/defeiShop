...
<div class="divContent">
  ...
  <c:if test="${af.map.type eq 100}"><c:set var="base_name" value="提现消费比例" /></c:if>
  ...
  <c:if test="${af.map.type eq 9000}"><c:set var="base_name" value="敏感词库" /></c:if>
   <c:if test="${af.map.type eq 1600}"><c:set var="base_name" value="工本费费用" /></c:if>
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
   ...
   <tr>
     <td width="14%" nowrap="nowrap" class="title_item">活动开始时间：</td>
     <td colspan="2">
       ${af.map.pre_varchar_1} </td>
   </tr>
   <tr>
     <td nowrap="nowrap" class="title_item">活动结束时间：</td>
     <td colspan="2">
       ${af.map.pre_varchar_2} </td>
   </tr>
    </c:if>
