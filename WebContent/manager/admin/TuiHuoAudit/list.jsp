		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tableClass">
			...
			<c:forEach var="cur" items="${entityList}" varStatus="vs">
				...
				<tr align="center">
							...
							<c:forEach var="oidsList" varStatus="vs1" items="${cur.map.orderInfo.orderInfoDetailsList}">
								<tr>
									...
									<td width="20%" align="center">
										<c:if test="${not empty oidsList.good_price}"> ￥
					...
					<td>
						<script type="text/javascript">showOrderTuihuoState('${cur.audit_state}');</script>
					</td>
...
<%-- 							<a class="label label-info label-block" href="../admin/TuiHuoAudit.do?method=audit&id=${cur.id}&mod_id=${af.map.mod_id}&par_id=${af.map.par_id}">审核</a> --%>
<%-- 						</c:if> --%>
						<c:if test="${empty af.map.expect_return_way_in}">			
			          ...
			          <c:if test="${cur.audit_state ne 1}">
						<a class="butbase"><span onclick="doNeedMethod(null, 'TuiHuoAudit.do', 'audit','id=${cur.id}&is_audit=1&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize())" class="icon-ok">审核</span></a>
		          		</c:if>
		          		</c:if>
				...
				<c:if test="${not empty cur.map.msglist}">
				<tr>
					<td colspan="7" style="background: #e6e6e6;">反馈内容</td>
				</tr>
				<tr>
					<c:forEach items="${cur.map.msglist}" var="item">
						<td colspan="6">${item.content}</td>
						<td align="center">
						<a class="butbase"><span onclick="doNeedMethod(null, 'TuiHuoAudit.do', 'view','id=${cur.id}&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize())" class="icon-ok">查看反馈</span></a>
						</td>
					</c:forEach>
				</tr>
				</c:if>
