...
<div align="center" class="divContent">
  ...
  <html-el:form action="/admin/OrderQuery"  styleClass="searchForm">
...
<!--                   &nbsp;<input id="download" type="button" value="导出" class="bgButton" /> -->
  ...
  <form id="listForm" name="listForm" method="post" action="OrderQuery.do?method=delete">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      <tr>
        ...
        <th width="16%" align="center"><strong>下单人信息</strong></th>

          <th width="5%" align="center"><strong>下单平台</strong></th>
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        ...
        <tr>
          <td align="left"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
              ...
              <c:set var="style" value="" />
                ...
                <tr style="${style}">
                  ...
                  <c:if test="${cur.order_type eq 100}">&nbsp;&nbsp;订单号：${cur.trade_index }</c:if><br />
          ...
          <c:if test="${empty cur.rel_name}">--</c:if>
            <c:if test="${cur.order_type ne 50 }">
             <a class="viewDetail" href="${ctx}/manager/admin/UserRelation.do?method=queryAllParList&user_id=${cur.add_user_id}&mod_id=1002005000">
          ...
          <td align="center">下单人姓名：${cur.add_user_name}<c:if test="${empty cur.add_user_name}">--</c:if>
          <br>下单人电话:${cur.map.add_user_mobile}<c:if test="${empty cur.map.add_user_mobile}">--</c:if>
           <br>下单时间:<fmt:formatDate value="${cur.order_date}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
          <td align="center">	           
           <c:forEach var="curPayPlatFormType" items="${payPlatformList}">
	              <c:if test="${curPayPlatFormType.index eq cur.pay_platform}">
	                ${curPayPlatFormType.name}
	              </c:if>
	            </c:forEach></td>
        ...
        <tr>
          <td align="left"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
              <c:set var="isPd" value="false" />
              <c:forEach var="oidsList" varStatus="vs1" items="${childEntity.orderInfoDetailsList}">
              <c:set var="style" value="" />
                  <c:if test="${oidsList.is_tuihuo eq 1}">
                  	<c:set var="style" value="text-decoration:line-through" />
                  </c:if>
                <tr style="${style}">
                  ...
                  <c:if test="${childEntity.order_type eq 100 and childEntity.is_leader eq 0}">团员订单</c:if>&nbsp;&nbsp;订单号：${childEntity.trade_index }<br />
                    <c:if test="${childEntity.order_type eq 10 or childEntity.order_type eq 30  or childEntity.order_type eq 40}">
                    <c:url var="url" value="/entp/IndexEntpInfo.do?method=getCommInfo&id=${oidsList.comm_id}" />
                    <a href="${url}" target="_blank">${oidsList.comm_name}</a>
                    </c:if>
                    <c:if test="${childEntity.order_type eq 50 }">${oidsList.comm_name}</c:if>
                    <c:if test="${childEntity.order_type eq 7 or childEntity.order_type eq 20}">${oidsList.comm_name}</c:if>
                    <c:if test="${not empty oidsList.comm_tczh_name}"> &nbsp;[${oidsList.comm_tczh_name}] </c:if>
                    <c:if test="${oidsList.fp_state eq 2}">&nbsp;<img src="${ctx}/styles/images/fapiao.png" title="发票已寄送" class="fapiao"/></c:if>
                    <c:if test="${oidsList.fp_state eq 1}">&nbsp;<img src="${ctx}/styles/images/no_fapiao.png" title="发票未寄送" class="fapiao"/></c:if>
                  </td>
          ...
          <td align="center">${childEntity.rel_phone}<c:if test="${empty childEntity.rel_phone}">--</c:if></td>
          <td align="center">${childEntity.add_user_name}<c:if test="${empty childEntity.add_user_name}">--</c:if></td>
          <td align="center">${childEntity.map.add_user_mobile}<c:if test="${empty childEntity.map.add_user_mobile}">--</c:if></td>
          <td align="center"><fmt:formatDate value="${childEntity.order_date}" pattern="yyyy-MM-dd" /></td>
          <td align="center" nowrap="nowrap">
          	第三方支付：￥<fmt:formatNumber value="${childEntity.order_money}" pattern="0.00"/><br/>
          	应付：￥<fmt:formatNumber value="${childEntity.no_dis_money}" pattern="0.00"/>
            <br/>
          ...
          <td align="center" nowrap="nowrap" id="order_state_${childEntity.id}" title="审核说明：${childEntity.map.orderAudit.audit_desc}" data-order-state="${childEntity.order_state}" data-pay-type="${childEntity.pay_type}" data-order-type="${childEntity.order_type}"><script type="text/javascript">showOrderState(${childEntity.order_state},${childEntity.pay_type},${childEntity.order_type})</script>
            <div>
            	<c:if test="${childEntity.order_type eq 100 and childEntity.is_group_success eq 0}" var="isziti">拼团未完成</c:if>
            </div>
            <div>
            	<c:if test="${childEntity.order_type eq 100 and childEntity.is_group_success eq 1}" var="isziti">拼团完成</c:if>
            </div>
            <div>
            ...
            <td align="center">
          	<c:choose>
          		<c:when test="${empty childEntity.is_price_modify}"><span style=" color:#060;">否</span></c:when>
          		<c:when test="${childEntity.is_price_modify eq 1}"><span style=" color:#F00;">是（修改前：<fmt:formatNumber value="${childEntity.price_modify_old}" pattern="0.##"/>元）</span></c:when>
          	</c:choose>
          </td>
          <c:if test="${isOffline eq true}">
	          <td align="center">
          ...
          <td align="center">
          <a href="?method=view&amp;mod_id=${af.map.mod_id}&amp;order_id=${childEntity.id}">订单详情</a>
          </td>
        </tr>
        </c:forEach>
        </c:if>
  ...
  <div class="pageClass">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="${ctx}/manager/admin/OrderQuery.do">
        ...
        <tr>
          <td height="10"><script type="text/javascript" src="${ctx}/commons/scripts/pager.js">;</script>
            <script type="text/javascript">
				...
				pager.addHiddenInputs("payPlatform", "${af.map.payPlatform}");
...
$(document).ready(function(){
	...
	$("#download").click(function(){
		...
		var submit = function (v, h, f) {
		    if (v == true) {
		    	location.href = "${ctx}/manager/admin/OrderQuery.do?method=toExcel&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
		    } else {
		    	location.href = "${ctx}/manager/admin/OrderQuery.do?method=toExcel&code=GBK&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
		    }
		    return true;
		};
	...
	$("#nav_ul_content a").click(function(){ 
		 ...
		 this.href= "${ctx}/manager/admin/OrderQuery.do?method=list&order_type="+ type + "&" + $('#bottomPageForm').serialize();
	     this.target = "_self";
	});
...
function updateStateFh(id){
	$.dialog({
		...
		height: 400,
        lock:true ,
		content:"url:${ctx}/manager/admin/OrderQuery.do?method=updateStateFh&order_id=" +id + "&" + $('#bottomPageForm').serialize()
	});
}
