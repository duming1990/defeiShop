...
<%--       <c:if test="${af.map.link_type eq 90}"> --%>
		<c:if test="${af.map.link_type ne 10070 }">
      	...
      	<c:if test="${af.map.link_type eq 10120}">
      	<tr>
          	 <td class="title_item" nowrap="nowrap" align="center" width="15%">内容</td>
<%--           	 <td colspan="2" width="5%" ><html-el:text property="comm_name" styleId="comm_name" maxlength="125" style="width:480px" styleClass="webinput" /></td> --%>
          <td colspan="2" width="85%"><html-el:textarea styleId="content" property="content" rows="7" style="width:597px; height:150px" /></td>
          </tr>
         </c:if>
         <c:if test="${af.map.link_type eq 10110}">
      	<tr>
          	 <td class="title_item" nowrap="nowrap" align="center" width="15%">内容</td>
<%--           	 <td colspan="2" width="5%" ><html-el:text property="comm_name" styleId="comm_name" maxlength="125" style="width:480px;" styleClass="webinput" /></td> --%>
			...
			<tr>
		        ...
		        <td colspan="2" width="85%">
		         <c:if test="${not empty af.map.pre_number}">
          			<img src="${ctx}/styles/indexEntp/images/img/${af.map.pre_number}.png" height="200" width="400px"/>
	          </c:if>
	          <c:if test="${empty af.map.pre_number}">暂无图片</c:if>
		         </td>
		      </tr>
          </c:if> 
      <c:if test="${af.map.link_type ne 10070}">
		        ...
		        <div class="files-warp" id="image_path_warp">
		          ...
		          <div style="color:rgb(241, 42, 34); padding: 5px;"> 
		              ...
		              <c:if test="${af.map.link_type eq 10}">[图片尺寸] 建议：[1920px * 60px] </c:if>
		         	  ...
		         	  <c:if test="${af.map.link_type eq 10080}"><span style="color:red">[图片尺寸] 建议：[1920px * 1080px]</span></c:if>
					<c:if test="${af.map.link_type eq 10010}"><span  style="color:red">[图片尺寸] 建议：[95px * 95px]</span></c:if>
					<c:if test="${af.map.link_type eq 10020}"><span style="color:red">[图片尺寸] 建议：[80px * 61px]</span></c:if>
					<c:if test="${af.map.link_type eq 10030}"><span style="color:red">[图片尺寸] 建议：[398px * 304px]</span></c:if>
					<c:if test="${af.map.link_type eq 10040}"><span style="color:red">[图片尺寸] 建议：[1696px * 862px]</span></c:if>
					<c:if test="${af.map.link_type eq 10060}"><span style="color:red">[图片尺寸] 建议：[198px * 65px]</span></c:if>
