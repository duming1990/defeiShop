...
<div class="divContent">
  ...
  <html-el:form action="/admin/BaseLinkNew">
  ...
  <form id="listForm" name="listForm" method="post" action="BaseLinkNew.do?method=delete">
    <div style="padding-bottom:5px;">
      ...
      <input type="button" name="add" id="add" class="bgButton" value="添 加" onclick="location.href='BaseLinkNew.do?method=add&mod_id=${af.map.mod_id}&link_type=${af.map.link_type}&pre_number=${af.map.pre_number}&par_id=${af.map.par_id}&type=${af.map.type}';" />
...
<%--          <c:if test="${af.map.link_type eq 10070 }"> --%>
<!--         <th width="15%"></th> -->
<%--         </c:if> --%>
        <th width="12%">链接地址</th>
        <th width="8%">添加时间</th>
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        <tr align="center">
          ...
          <td align="left"><a href="BaseLinkNew.do?method=view&amp;id=${cur.id}&mod_id=${af.map.mod_id}">
          ...
          <td align="center">
          <c:if test="${not empty cur.pre_number}">
          	<img src="${ctx}/styles/indexEntp/images/img/${cur.pre_number}.png" height="200" width="400px"/>
          </c:if>
          <c:if test="${empty cur.pre_number}">暂无图片</c:if>
          </td>
          </c:if> 
            
            <c:if test="${af.map.link_type ne 10000 }">
          <td align="center">
          ...
          <td nowrap="nowrap">
            <a class="butbase" id="edit" onclick="confirmUpdate(null, 'BaseLinkNew.do', 'id=${cur.id}&' + $('#bottomPageForm').serialize())"><span class="icon-edit">修改</span></a>
          	<c:if test="${cur.is_del eq 0}">
          	    <a class="butbase" id="remove" onclick="confirmDelete(null, 'BaseLinkNew.do', 'id=${cur.id}&' + $('#bottomPageForm').serialize())"><span class="icon-remove">删除</span></a>
            </c:if>
  ...
  <div class="pageClass">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="BaseLinkNew.do">
