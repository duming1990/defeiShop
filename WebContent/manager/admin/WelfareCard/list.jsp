...
<div class="divContent">
  <html-el:form action="/admin/WelfareCard" styleClass="searchForm">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        <tr align="center">
          ...
          <td>
          	<a class="viewDetail" onclick="getApply(${cur.card_apply_id});">
          		${fn:escapeXml(cur.own_service_name)}
          	</a>
          </td>
          ...
          <td>
          <a class="butbase" onclick="getCardList(${cur.id});"><span class="icon-search">详情</span></a> 
          <logic-el:match name="popedom" value="+8+"> 
	          <c:if test="${cur.is_del ne 1}">
	             <a class="butbase" id="remove" onclick="stop(${cur.id},1);"><span class="icon-remove">停用</span></a>
	          </c:if>
	          <c:if test="${cur.is_del eq 1}">
	             <a class="butbase" id="remove" onclick="stop(${cur.id},0);"><span class="icon-ok">恢复</span></a>
	          </c:if>
          </logic-el:match>
          </td>
        </tr>
...
function stop(gen_id,is_del){
	if(confirm("确定执行该操作吗？")){
		location.href="${ctx}/manager/admin/WelfareCard.do?method=delete&gen_id="+gen_id+"&is_del="+is_del+"&mod_id=${af.map.mod_id}&" + $('#bottomPageForm').serialize();
	}	 
}                                   
                                          
function getApply(id){
	var url="${ctx}/manager/admin/WelfareCard.do?method=getApply&mod_id=1003002000&id="+id;
	$.dialog({
		title:  "福利卡申请信息",
		width: "80%",
		height: "90%",
...
function getCardList(id){
	var url="${ctx}/manager/admin/WelfareCard.do?method=cardList&mod_id=1003002000&gen_id="+id;
	$.dialog({
		title:  "福利卡信息",
		width: "80%",
		height: "90%",
