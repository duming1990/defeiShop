    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        <tr align="center">
          ...
          <td>
	          <c:if test="${cur.card_state eq -1}">作废</c:if>
	          <c:if test="${cur.card_state eq 0}">未激活</c:if>
	          <c:if test="${cur.card_state eq 1}">已激活</c:if>
	          <c:if test="${cur.card_state eq 2}">已用完</c:if>
          </td>
          <td>
...
$("#download").click(function(){
	...
	var submit = function (v, h, f) {
	    if (v == true) {
	    	location.href = "${ctx}/manager/admin/WelfareCard.do?method=toExcel&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
	    } else {
	    	location.href = "${ctx}/manager/admin/WelfareCard.do?method=toExcel&code=GBK&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
	    }
	    return true;
	};
...
function stop(id,is_del){
	if(confirm("确定执行该操作吗？")){
		$.ajax({
			url: "${ctx}/manager/admin/WelfareCard.do" , 
			data:"method=stop&id=" + id + "&is_del=" + is_del,
			dataType: "json", 
	        async: true, 
