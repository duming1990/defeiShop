...
<div style="width: 99%" class="divContent">
  ...
  <html-el:form action="/admin/MoneyAidUserQuery" styleClass="searchForm">
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClassSearch">
    <tr>
      <th><span style="color: #F00;"></span>提现申请总金额： ${uma.cash_count}</th>
    </tr>
    <tr>
      <th><span style="color: #F00;"></span>实际付款总金额： ${uma.cash_pay}</th>
    </tr>
  </table>
  <%@ include file="/commons/pages/messages.jsp" %>
  <form id="listForm" name="listForm" method="post" action="MoneyAidUserQuery.do?method=delete">
    <input type="hidden" name="mod_id" value="${af.map.mod_id}" />
    <div style="padding-bottom:5px;">
      <logic-el:match name="popedom" value="+8+">
<!--         <input type="button" name="audit" id="audit" class="bgButton" value="批量审核" onclick="confirmAuditAll(this.form);"></input> -->
        <input type="button" class="bgButton" value="打款" onclick="payForm();"></input>
      </logic-el:match>
<%--       <logic-el:match name="popedom" value="+17+"> --%>
<!--         <input type="button" name="pay" id="pay" class="bgButton" value="批量付款" onclick="confirmPayAll(this.form);"></input> -->
<%--       </logic-el:match> --%>
    </div>
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      ...
      <c:forEach var="cur" items="${userMoneyApplyList}" varStatus="vs">
        <tr>
          ...
          <td align="center">${fn:escapeXml(cur.cash_count_before)}余额</td>
...
<%--           <span align="center" style="color: red">手续费：${fn:escapeXml(cur.cash_rate)}元</span> --%>
          ...
          <c:if test="${isContains}" >
            <td align="center"><c:if test="${cur.info_state eq 0}">
                <logic-el:match name="popedom" value="+8+"> <a class="butbase" onclick="doNeedMethod(null, 'MoneyAidUserQuery.do', 'audit','mod_id=${af.map.mod_id}&id=${cur.id}&'+$('#bottomPageForm').serialize())"><span class="icon-ok">审核</span></a> </logic-el:match>
...
$("#download").click(function(){
		...
		var submit = function (v, h, f) {
		    if (v == true) {
		    	location.href = "${ctx}/manager/admin/MoneyAidUserQuery.do?method=toExcel&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
		    } else {
		    	location.href = "${ctx}/manager/admin/MoneyAidUserQuery.do?method=toExcel&code=GBK&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
		    }
		    return true;
		};
...
function payForm(){
	var url = "MoneyAidUserQuery.do?method=sendAidMoney";
	$.dialog({
		title:  "发放扶贫金",
		width:  600,
		height: 350,
		...
		$.jBox.alert("请至少选择一个审核项！", '提示');
			...
			var submit = function (v, h, f) {
			   ...
			   $.jBox.tip("执行中...", 'loading');
			     ...
			     $.post("MoneyAidUserQuery.do?method=auditAll&info_state="+info_state + "&audit_memo=" + audit_memo + "&" + $(form).serialize() ,{},function(data){
	...
	if (checkedCount == 0) {
		...
		var submit = function (v, h, f) {
		    if (v == true) {
		     $.jBox.tip("执行中...", 'loading');
		     window.setTimeout(function () { 
		     $.post("MoneyAidUserQuery.do?method=payAll"+"&" + $(form).serialize(),function(data){
		     //$.jBox.tip(data, "success");
		     location.reload();
		     });
		     }, 1000);
...
function confirmAudit(id){
	if(confirm("确认以及给申请人付过款？")){
		location.href="MoneyAidUserQuery.do?method=pay&id=" + id+"&mod_id=${af.map.mod_id}";
	}
}
function confirmTk(id){
	var submit = function (v, h, f) {
	    if (v == true) {
	    	 ...
	    	 $.jBox.tip("执行中...", 'loading');
		     window.setTimeout(function () { 
		    	 location.href = "${ctx}/manager/admin/MoneyAidUserQuery.do?method=confirmTk&id=" + id+"&mod_id=${af.map.mod_id}" + "&tuikuan_memo=" + tuikuan_memo;
		     }, 1000);
	    }
	    return true;
	};
