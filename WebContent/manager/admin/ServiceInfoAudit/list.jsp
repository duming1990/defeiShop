...
<div style="width: 99%" class="divContent">
  ...
  <html-el:form action="/admin/ServiceInfoAudit" styleClass="searchForm">
  ...
  <form id="listForm" name="listForm" method="post" action="ServiceInfoAudit.do?method=delete">
  <div style="padding-bottom:5px;">
    <logic-el:match name="popedom" value="+1+">
      <input type="button" name="add" id="add" class="bgButton" value="添 加" onclick="location.href='ServiceInfoAudit.do?method=add&mod_id=${af.map.mod_id}';" />
    </logic-el:match>
    </div>
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClass">
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        <tr>
          ...
          <td align="left"><a title="查看" href="ServiceInfoAudit.do?method=view&amp;mod_id=${af.map.mod_id}&amp;id=${cur.id}">${fn:escapeXml(cur.servicecenter_name)}</a></td>
          <td align="center">
          ...
          <button class="label label-success"><a href="${ctx}/manager/admin/ServiceInfoAudit.do?method=indexTsg&mod_id=${af.map.mod_id}&link_id=${cur.id}" class="beautybg" style="color: white;">PC端扶贫馆LOGO维护</a></button>
          <button class="label label-primary"><a href="${ctx}/IndexTsg.do?method=index&Link_id=${cur.id}" target="_blank" style="color: white;">预览</a></button>
           <button class="label label-success"><a href="${ctx}/manager/admin/ServiceInfoAudit.do?method=indexMTsg&mod_id=${af.map.mod_id}&link_id=${cur.id}" class="beautybg" style="color: white;">M端扶贫馆LOGO维护</a></button>
          <button class="label label-primary"><a href="${ctx}/m/IndexMTsg.do?method=index&link_id=${cur.id}" target="_blank" style="color: white;">预览</a></button>
          </c:if>
          </td>
          <td align="center" nowrap="nowrap">
            <logic-el:match name="popedom" value="+8+">
                <c:if test="${cur.audit_state ne 1}"> <a class="butbase" onclick="doNeedMethod(null, 'ServiceInfoAudit.do', 'audit','id=${cur.id}&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize())"><span class="icon-ok">审核</span></a> </c:if>
            ...
            <div style="padding-top: 2px;">
              <logic-el:match name="popedom" value="+2+">
			    <a class="butbase" onclick="confirmUpdate(null, 'ServiceInfoAudit.do', 'id=${cur.id}&mod_id=${af.map.mod_id}&' + $('#bottomPageForm').serialize())"><span class="icon-edit">修改</span></a>
              </logic-el:match>
              ...
              <logic-el:match name="popedom" value="+4+">
                <c:if test="${cur.audit_state ne 1}"><a class="butbase" onclick="confirmDelete(null, 'ServiceInfoAudit.do', 'id=${cur.id}&mod_id=${af.map.mod_id}&' + $('#bottomPageForm').serialize())"><span class="icon-remove">删除</span></a> </c:if>
            ...
            <logic-el:match name="popedom" value="+19+">
              ...
              <div style="padding-top: 2px;">
                ...
                <c:if test="${cur.effect_state eq 1}"><a class="butbase" onclick="doNeedMethod(null, 'ServiceInfoAudit.do', 'cancelFuWu','id=${cur.id}&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize())"><span class="icon-cancel">取消合伙人</span></a> </c:if>
              </div>
              <div style="padding-top: 2px;">
          		<a class="butbase" onclick="add_Brougth_Account('${cur.id}')"><span class="icon-edit">增加对公账户</span></a>
              </div>
              <div style="padding-top: 2px;">
                <c:if test="${cur.effect_state ne 1}"><a class="butbase but-disabled" title="未确认，不能设置"><span class="icon-edit">设置二级域名</span></a> </c:if>
                <c:if test="${cur.effect_state eq 1}"><a class="butbase" onclick="addDomainSite('${cur.id}')"><span class="icon-edit">设置二级域名</span></a> </c:if>
              </div>
            </logic-el:match>
          </td>
        </tr>
  ...
  <div class="pageClass">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="ServiceInfoAudit.do">
...
$("#download").click(function(){
	...
	var submit = function (v, h, f) {
	    if (v == true) {
	    	location.href = "${ctx}/manager/admin/ServiceInfoAudit.do?method=toExcel&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
	    } else {
	    	location.href = "${ctx}/manager/admin/ServiceInfoAudit.do?method=toExcel&code=GBK&mod_id=${af.map.mod_id}&" + $('.searchForm').serialize();
	    }
	    return true;
	};
...
function setPayInfo(id){
	var url = "ServiceInfoAudit.do?method=setPayInfo&id=" + id;
	$.dialog({
		title:  "设置支付信息",
		width:  900,
		height: 520,
...
function add_Brougth_Account(id){
	var url = "ServiceInfoAudit.do?method=add_Brougth_Account&id=" + id;
	$.dialog({
		title:  "增加对公账户",
		width:  900,
		height: 520,
...
function addDomainSite(id){
	var url="ServiceInfoAudit.do?method=addDomainSite&id="+id;
	$.dialog({
		titile:"设置二级域名",
		width:900,
		height:520,
...
function confirmServiceCenter(id){
	var submit2 = function(v, h, f) {
		if (v == "ok") {
			...
			location.href="ServiceInfoAudit.do?method=confirmServiceInfo&id="+ id +"&mod_id=${af.map.mod_id}&" + $('#bottomPageForm').serialize();
		}
	};
...
function cancelServiceCenter(id){
	var submit2 = function(v, h, f) {
		if (v == "ok") {
			$.jBox.tip("操作中...", 'loading');
			location.href="ServiceInfoAudit.do?method=cancelServiceInfo&id="+ id +"&mod_id=${af.map.mod_id}&" + $('#bottomPageForm').serialize();
		}
	};
