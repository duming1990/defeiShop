...
(function($) {
	...
	$.extend( {
		add2cartbuynow : function(source_id, url) {
			...
			var price_ref = datas[10];
			var comm_tczh_name = $("#comm_tczh_name").val();
			...
			if(pd_price==''||pd_price==null||undefined == pd_price){
				pd_price = price_ref;
			}
			...
			$.ajax({
				type: "POST",
				url: "CsAjax.do",
				data: "method=saveCartInfo&pd_id=" + pd_id + "&pd_price="  + pd_price + "&pd_name=" +pd_name+ "&cls_id="  + cls_id + "&cls_name=" + cls_name + "&cart_type=" + cart_type + "&entp_id=" + entp_id + "&comm_id=" + comm_id + "&comm_name=" + comm_name + "&comm_tczh_id=" + comm_tczh_id + "&comm_tczh_price=" + comm_tczh_price + "&comm_tczh_name=" + comm_tczh_name + "&delivery_p_index=" + delivery_p_index + "&comm_services=" + comm_services + "&comm_weight=" + comm_weight + "&gm_pd_count=" + pd_count + "&fre_id=" + freight_id ,
				dataType: "json",
				error: function(request, settings) {alert("\u6570\u636e\u52a0\u8f7d\u8bf7\u6c42\u5931\u8d25\uff01"); },
				success: function(data) {
					if (data.result) {
						location.href = url + "-" + data.result + ".shtml";
					} else{
						var tip = data.tip;
						if(tip!=null&&tip!=''){
							alert(tip);
			...
			if (!shadow.attr('id')) {
				$('body').prepend(
						'<div id="' + source.attr('id')
								+ '_shadow" class="flyEffect">' + comm_name
								+ '</div>');
	        		...
	        		if (data.result) {
	        			shadow.animate( {
	        				...
	        				if ($('#li_'+comm_tczh_id).length <=0){
	        					var htmldata = '<li id="li_'+comm_tczh_id+'"><span><a class="del" onclick="delCart(this,'+data.result+');">\u5220\u9664</a></span><span class="cartcount" id="pd_count_'+comm_tczh_id+'">'+pd_count+'</span><span> ￥'+pd_price+'&nbsp;×</span><font class="ofCart" title="'+comm_name+'">'+comm_name+'</font></li>';
