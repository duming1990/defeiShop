...
(function(a) {
	a.jBox = function(b, c) {
		...
		var e = a('#' + c.id,top.document.body);
		if (e.length > 0x0) {
		...
		if (!c.isTip && !c.isMessager && !c.showScrolling) {
			a(a.browser.msie ? 'html' : 'top.document.body').attr('style', 'overflow:hidden;padding-right:17px;')
		};
		var g = !c.isTip && !(c.title == undefined);
		var h = f.type == 'GET' || f.type == 'POST' || f.type == 'IFRAME';
		var i = typeof c.width == 'number' ? (c.width - 0x32) + 'px' : "90%";
		var j = [];
		j.push('<div id="' + c.id + '" class="jbox-' + (c.isTip ? 'tip' : (c.isMessager ? 'messager' : 'top.document.body')) + '">');
		if (c.showFade) {
			if ((d && a('iframe',top.document.body).length > 0x0) || a('object, applet',top.document.body).length > 0x0) {
				j.push('<iframe id="jbox-fade" class="jbox-fade" src="about:blank" style="display:block;position:absolute;z-index:-1;"></iframe>')
			} else {
				if (d) {
					a('select',top.document.body).css('visibility', 'hidden')
				};
		...
		if (!f.isObject) {
			switch (f.type) {
			case "ID":
				f.html = a('#' + b,top.document.body).html();
				break;
			case "GET":
			case "POST":
				f.html = '';
			...
			case "IFRAME":
				f.html = k;
				if (b.indexOf('#') == -0x1) {
					f.url = b + (b.indexOf('?') == -0x1 ? '?t' : '&t') + new Date().getTime();
				} else {
					var N = b.split('#');
		...
		var w = function() {
				var N = l.width();
				return top.document.body.clientWidth < N ? N : top.document.body.clientWidth
			};
		var x = function() {
				var N = l.height();
				return top.document.body.clientHeight < N ? N : top.document.body.clientHeight
			};
		var y = function() {
				if (!c.showFade) {
					return
		...
		var I = function() {
				if (!c.isTip && !c.isMessager) {
					if (a('.jbox-body',top.document.body).length == 0x1) {
						a(a.browser.msie ? 'html' : 'top.document.body').removeAttr('style')
					};
					J()
				} else {
					if (c.isTip) {
						var tip = a(top.document.body).data('tip');
	...
	a.jBox.getBox = function() {
		return a('.jbox-body',top.document.body).eq(a('.jbox-body',top.document.body).length - 0x1)
	};
	a.jBox.getIframe = function(b) {
		var c = (typeof b == 'string') ? a('#' + b,top.document.body) : a.jBox.getBox();
	...
	a.jBox.close = function(b, c) {
		b = b || false;
		c = c || 'top.document.body';
		if (typeof b == 'string') {
			a('#' + b,top.document.body).find('.jbox-close').click()
		} else {
			var d = a('.jbox-' + c,top.document.body);
			if (b) {
	...
	a.jBox.tip = function(b, c, d) {
		...
		e.push('</div>');
		d.content = e.join('');
		if (a('.jbox-tip',top.document.body).length > 0x0) {
			a(top.document.body).data('tip', {
				next: true,
				options: d
			});
		...
		if (d.focusId != undefined) {
			a('#' + d.focusId,top.document.body).focus();
			top.$('#' + d.focusId).focus()
		};
		a.jBox(d.content, d)
	};
...
importJboxCss();
function importJboxCss() {
	var head = top.document.getElementsByTagName('head')[0];
	var scripts = document.getElementsByTagName('script');
	for (var i = 0; i < scripts.length; ++i) {
		var src = scripts[i].src;
		if (src) {
			var index = src.indexOf("jbox");
			if (index >= 0) {
				index = src.indexOf("jBox");var webRoot = src.substring(0, index);var abcss = top.document.createElement('link'); abcss.type = 'text/css'; abcss.rel = "stylesheet";abcss.href = webRoot + "jBox/Skins/Gray/jbox.css";head.appendChild(abcss);
				break;
			}
		}
	}
